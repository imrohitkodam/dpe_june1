...
class com_NobossfaqInstallerScript{
	...
	function update($parent) {
        // Criado em Agosto 2024 para novas colunas relacionadas a opcoes de conteudos da FAQ
        $this->addColumnOptionsContent();
    ...
    function uninstall($parent){
        ...
        if(is_dir(JPATH_LIBRARIES.'/noboss/src')){
            ...
            // $packageExtensions = Noboss\Library\Util\NbInstallScriptUtil::getPackageExtensions('pkg_nobossfaq');
            
            // // Existem extensoes vinculadas (faq, testimonials, blog)
            // if(count($packageExtensions) > 0) {
            //     // Remove as extensoes vinculadas
            //     Noboss\Library\Util\NbInstallScriptUtil::uninstallPackageExtensions($packageExtensions);
            // }
        }
        // Library ainda esta no formato antigo sem pasta /src
        else if(is_dir(JPATH_LIBRARIES.'/noboss/util')){
            ...
            // // Verifica extensoes vinculadas ao pacote
            // $packageExtensions = NoBossUtilInstallscript::getPackageExtensions('pkg_nobossfaq');
    ...
    // Criado em Junho 2024 para novas colunas relacionadas a opcoes de conteudos da FAQ
    public static function addColumnOptionsContent(){
        $db = JFactory::getDbo();

        // INICIO: VERIFICA SE uma das NOVAS COLUNAS JA EXISTE NO BANCO ANTES DE SEGUIR COM ATUALIZACAO
        ...
        $query = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='{$dbprefix}noboss_faq_group' AND COLUMN_NAME = 'content_display_registered'";
        ...
        $query = $db->getQuery(true);
        $query = "ALTER TABLE  `#__noboss_faq_group` 
        ADD COLUMN `content_display_registered` tinyint(3) DEFAULT 1 AFTER `config_module_faqs_display`,
        ADD COLUMN `content_display_articles` tinyint(3) DEFAULT 0 AFTER `content_display_registered`, 
        ADD COLUMN `content_categories_articles` text DEFAULT NULL AFTER `content_display_articles`; ";
