...
class NobossfaqViewQuestions extends NobossComponentsViewList {
	/**
     * Metodo principal
     */
	public function display($tpl = null) {
        // Nome da view de edicao dos registros
        $this->createViewAlias = 'question';
        
        // Alias do campo de id do componente
        $this->recordIdAlias = 'id_faq';
        
        // Titulo da pagina
        $this->pageTitle = 'No Boss FAQ: '.JText::_('COM_NOBOSSFAQ_QUESTION');
        
        // Icone exibico junto ao titulo da pagina Z(sem informar prefixo 'icon-')
        $this->pageIcon = 'help questions';
    ...
    public function columnsDisplay(){
        ...
        $this->mainColumn = 'question';
        // Titulo da coluna princpal (informar apenas a constante)
        $this->mainColumnName = 'COM_NOBOSSFAQ_QUESTION';
        ...
        $column = new stdClass();
        // Alias da coluna
        $column->alias = 'group_name';
        // Titulo da coluna (informar apenas a constante)
        $column->title = 'COM_NOBOSSFAQ_GROUP_NAME';
        ...
        $column = new stdClass();
        // Alias da coluna
        $column->alias = 'category_name';
        // Titulo da coluna (informar apenas a constante)
        $column->title = 'JCATEGORY';
        // Flag p/ determinar se coluna permitira ordenacao
        $column->allowOrdering = 1;
        // Html de retorno (pode colocar html setando '%VALUE%' para onde deve ser inserido o valor do item)
        $column->returnHtml = "%VALUE%";
    ...
    public function displayEmptyState($emptyState = array()){
        ...
        // Titulo
        $emptyState['title'] = JText::_('COM_NOBOSSFAQ_QUESTION_EMPTY_STATE_TITLE');
        // Texto de apoio
        $emptyState['content'] = JText::_('COM_NOBOSSFAQ_QUESTION_EMPTY_STATE_CONTENT');
        // Texto do botao de adicionar novo item
        $emptyState['btnadd'] = JText::_('COM_NOBOSSFAQ_QUESTION_EMPTY_STATE_BTN_ADD');
        // Icone
    ...
    public function specificTreatments(){       
		...
		if(!$this->model->isGroupPublished()){
            ...
            $this->noticeIntro = str_replace("#link#", JRoute::_('index.php?option=com_nobossfaq&view=groups'), JText::_('COM_NOBOSSFAQ_ALERT_NO_GROUPS'));
        }
    }
}
