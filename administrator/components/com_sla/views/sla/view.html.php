...
class SlaViewSla extends HtmlView
{
	...
	public function display($tpl = null)
	{
		...
		$this->canDo = ContentHelper::getActions('com_sla', 'sla', $this->item->id);
	...
	protected function addToolbar()
	{
		...
		if ($layout != "default")
		{
			...
			ToolbarHelper::title(
				Text::_('COM_SLA_PAGE_' . ($checkedOut ? 'VIEW_SLA' : ($isNew ? 'ADD_SLA' : 'EDIT_SLA'))),
				'pencil-2 sla-add'
			);
			...
			if ($isNew)
			{
				...
				ToolbarHelper::save('sla.save');
				ToolbarHelper::cancel('sla.cancel');
			}
			else
			{
				...
				ToolbarHelper::cancel('sla.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			ToolbarHelper::title(
				Text::_('COM_SLA_PAGE_VIEW_SLA')
			);
