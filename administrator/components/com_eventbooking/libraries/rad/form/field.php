...
abstract class RADFormField
{
	 ...
	 *
	 * @param   stdClass  $row  the table object store form field attribute
	 * @param   mixed     $value
	 ...
	 */
	public static function create($row, $value = null, $fieldSuffix = null)
	{
		$class = 'RADFormField' . ucfirst($row->fieldtype);

		if (class_exists($class))
		{
			return new $class($row, $value, $fieldSuffix);
		}
	...
	public function getControlGroup($bootstrapHelper = null, $controlId = null)
	{
		...
		if ($this->row->container_size && Factory::getApplication()->getInput()->getCmd('view') === 'register')
		{
	...
	public function getOutput($tableLess = true, $bootstrapHelper = null)
	{
		...
		if ($tableLess)
		{
			...
			return '<div class="' . $controlGroupClass . ' eb-field-value field-' . $this->name . '-container"><div class="' . $controlLabelClass . '">' . $this->title . '</div><div class="' . $controlsClass . '">' .
				$fieldValue . '</div></div>';
		}

		return '<tr class="field-' . $this->name . '-container"><td class="title_cell">' . $this->title . '</td><td class="field_cell">' . $fieldValue . "</td></tr>\r\n";
	}
	...
	public function makeFieldOptional()
	{
		...
		if (str_contains($cssClass, 'validate[required,'))
		{
		...
		if (str_contains($cssClass, ' validate[]'))
		{
	...
	{
		if (str_contains($validationRules, 'validate[required,'))
		{
		...
		if (str_contains($validationRules, 'validate[]'))
		{
