...
/**
 * Class supporting a list of Advsearch records.
 *
 * @since  3.3
 */
class AdvsearchModelstat extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	{
		$db    = JFactory::getDBO();
		$query = "select * from #__advanced_search_indexer order by id ASC";
		$db->setQuery($query);
		$IndexerData = $db->loadObjectlist();
		$i           = 1;
		$Data = array();

		foreach ($IndexerData as $k => $v)
		{
			$query = "select count(id) FROM " . $db->quoteName('#__' . $v->mapped_table);
			$db->setQuery($query);
			$RecordCount             = $db->loadResult();
			$Data[$k]['recordcount'] = $RecordCount;
			$Data[$k]['name']        = $v->name;
			$i++;
		}
