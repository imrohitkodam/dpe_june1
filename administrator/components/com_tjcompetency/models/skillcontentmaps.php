 ...
 * @since  1.0.0
 */
class TjCompetencyModelSkillContentMaps extends ListModel
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select(array('a.*', 'IF(users.name IS NULL,"' . Text::_('COM_TJCOMPETENCY_BLOCKED_USER') . '",users.name) AS uname', 'b.title as skill_title', 'c.title as scale_title'));
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(b.title LIKE ' . $search . ' OR c.title LIKE ' . $search . ')');
			}
		}
