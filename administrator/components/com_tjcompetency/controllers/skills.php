 ...
 * @since  1.0.0
 */
class TjCompetencyControllerSkills extends AdminController
{
	const CSV_FILE_LOCATION     = JPATH_SITE . '/media/com_tjcompetency/csvimport/';

	public $csvProgressFilePath = self::CSV_FILE_LOCATION . 'skill_progress.txt';
	 ...
	 * @since  1.0.0
	 */
	public function getModel($name = 'Skill', $prefix = 'TjCompetencyModel')
	{
	...
	public function rebuild()
	{
		...
		$this->setRedirect(JRoute::_('index.php?option=com_tjcompetency&view=skills', false));
		...
		{
			// Rebuild succeeded.
			$this->setMessage(JText::_('COM_TJCOMPETENCY_REBUILD_SKILL_SUCCESS'));

			return true;
		}

		// Rebuild failed.
		$this->setMessage(JText::_('COM_TJCOMPETENCY_REBUILD_SKILL_FAILURE'));

		return false;
	}

	/**
	 * CSV file data store in Skills
	 *
	 * @return  void|string
	 *
	 * @since   1.0.0
	...
	public function csvImport()
	{
		...
		$logFileName = 'com_tjcompetency.skills_import_' . $dateTime . '.log';
		...
		$session->set('skills_log_filename', $logFileName);
		...
		if ($fileToUpload['name'] != '')
		{
			...
			{
				$fileName    = File::makeSafe(rand()) . '_skills.' . $ext;
				$uploadPath  = self::CSV_FILE_LOCATION . $fileName;

				$session->set('skills_filename', $fileName);
	...
	public function csvProcess()
	{
		...
		$session  = Factory::getSession();
		$fileName = $session->get('skills_filename');
