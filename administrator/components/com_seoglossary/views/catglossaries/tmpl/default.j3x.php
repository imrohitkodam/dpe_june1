...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_seoglossary&task=catglossaries.saveOrderAjax&tmpl=component';
...
<form action="<?php echo JRoute::_('index.php?option=com_seoglossary&view=catglossaries');?>" method="post" name="adminForm" id="adminForm">
		...
		<table class="table table-striped" id="categoryList">
			<thead>
				<tr>
					...
					<th>
						<?php echo JHtml::_('grid.sort', 'COM_SEOGLOSSARY_CATGLOSSARIES_NAME', 'a.name', $listDirn, $listOrder); ?>
					</th>
			...
			<tbody>
				...
				foreach ($this->items as $i => $item) :
					...
					$canEdit    = $user->authorise('core.edit',       $extension . '.catglossary.' . $item->id);
					$canCheckin = $user->authorise('core.admin',      'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
					$canEditOwn = $user->authorise('core.edit.own',   $extension . '.catglossary.' . $item->id) && @$item->created_user_id == $userId;
					$canChange  = $user->authorise('core.edit.state', $extension . '.catglossary.' . $item->id) && $canCheckin;
				...
				<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo @$item->parent_id;?>" item-id="<?php echo $item->id?>" parents="<?php echo $parentsStr?>" level="<?php echo @$item->level?>">
					...
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, 'catglossaries.', $canChange);?>
					</td>
					<td>
						<?php //echo str_repeat('<span class="gi">&mdash;</span>', $item->level - 1) ?>
						<?php if ($item->checked_out) : ?>
							<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'catglossaries.', $canCheckin); ?>
						<?php endif; ?>
						<?php if ($canEdit || $canEditOwn) : ?>
							<a href="<?php echo JRoute::_('index.php?option=com_seoglossary&task=catglossary.edit&id='.$item->id);?>">
						...
						<br />
						<span class="small">
							<?php if (!empty($item->alphabet)) : ?>
								<?php echo $this->escape($item->alphabet);?>
							<?php endif; ?>
						</span>
					</td>
