...
class ComDocmanControllerBehaviorSyncable extends KControllerBehaviorAbstract
{
        ...
        else if ($name === 'folder')
        {
            foreach ($context->result as $entity)
            {
                ...
                if ($entity->path)
                {
                    ...
                    $this->_setDefaultCategoryFolder($entity->path);
                }
        ...
        foreach ($this->_path_cache as $row) {
            ...
            if ($name === 'folder')
            {
                ...
                $query = $this->getObject('database.query.update')
                    ...
                    ->where('folder LIKE :filter1')
                    ->where('folder LIKE :filter2', 'OR')
                    ->bind(array('to' => $to_path, 'from' => $from_path, 'filter1' => $from_path, 'filter2' => $from_path.'/%'));
    ...
    public function syncFiles()
    {
        ...
        $iterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($path, FilesystemIterator::FOLLOW_SYMLINKS), RecursiveIteratorIterator::CHILD_FIRST);
        $insert   = array();
        ...
        if (count($insert))
        {
            ...
            for ($i = 0, $count = count($insert); $i < $count; $i++)
            {
                ...
                $query->values(array_merge($this->_splitPath($file), [$this->getModifiedTime($path.'/'.$file)]));
    ...
    /**
     * Sets defaut folder of the category for the deleted folder
     *
     * @param string $folder
     */
    protected function _setDefaultCategoryFolder($folder)
    {
        $relation = $this->_getRelation($folder);
     ...
     * @return mixed
     */
    protected function _getRelation($folder)
    {
        $relation = $this->getObject('com://admin/docman.database.table.category_folders')
                         ->select(array('folder' => $folder), KDatabase::FETCH_ROW);
