...
class ComDocmanControllerDocument extends ComKoowaControllerModel
{
    public function __construct(KObjectConfig $config)
    {
        ...
        $this->addCommandCallback('after.browse', '_showNotices');
    }
    ...
    protected function _initialize(KObjectConfig $config)
    {
        $config->append(array(
            ...
            'behaviors' => array(
                ...
                'scannable',
                'com://admin/docman.controller.behavior.exportable.csv',
                'revisable',
                'restrictable' => array('redirect_url' => 'index.php?option=com_docman&view=config')
            )
        ));
     ...
     * @param KControllerContextInterface $context
     */
    protected function _showNotices(KControllerContextInterface $context)
    {
        $category = $context->request->query->category;

        if (!is_null($category) && !$this->getObject('com:docman.model.categories')->id($category)->count())
        {
            $translator = $this->getObject('translator');
            $message = $translator->translate('The category you selected does not or no longer exists');
    ...
    protected function _actionRefresh(KControllerContextInterface $context)
    {
        ...
        $translator = $this->getObject('translator');
        $message = $translator->translate('The files and folders cache has been refreshed');
        $context->response->addMessage($message, KControllerResponse::FLASH_NOTICE);
    }
}
