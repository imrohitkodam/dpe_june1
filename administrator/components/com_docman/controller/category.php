...
class ComDocmanControllerCategory extends ComKoowaControllerModel
{
    ...
    /**
     * If the category parent id is supplied in the URL, prepopulate it in the new category form
     *
    ...
    protected function _populateCategory(KControllerContextInterface $context)
    {
        ...
        {
            $parent_id = $this->getRequest()->query->parent_id;
            $view      = $this->getView();

            if ($this->getRequest()->getFormat() === 'html' && $view->getName() == 'category' && !empty($parent_id)) {
                $this->getView()->set('parent_id', $parent_id);
            }
        }
    }
}
