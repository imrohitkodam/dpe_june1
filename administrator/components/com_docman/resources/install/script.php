...
class com_docmanInstallerScript extends JoomlatoolsInstallerHelper
{
    ...
    public function afterInstall($type, $installer)
    {
        ...
        if ($type === 'update')
        {
            ...
            // Backward compatibility for doclink embedded player created prior v5.x
            if ($this->old_version && version_compare($this->old_version, '5.0.0', '<'))
            {
                $config->doclink_show_player = $config->getDoclinkParams('show_player');
                $config->doclink_show_icon   = $config->getDoclinkParams('show_icon');
                $config->doclink_show_size   = $config->getDoclinkParams('show_size');
            }
        }

        // Default duplicate mode upon install or update
        $config->update_mode = 'confirm';

        // Default CSV export fields
        $config->csv_fields = array(
            array('uuid' => 1),
            array('title' => 1),
            array('enabled' => 1),
            array('access_title' => 1),
            array('created_by' => 1),
            array('created_on' => 1),
            array('category_title' => 1),
            array('hits' => 1),
            array('image_path' => 1),
            array('document_url' => 1),
            array('download_url' => 1),
            array('permalink' => 1)
        );
        ...
        if (count($incompatible)) {
            ...
            \Joomla\CMS\Factory::getApplication()->enqueueMessage(sprintf($warning, implode(' and ', $incompatible)), 'warning');
        }
    }
    ...
    public function update($installer)
    {
        ...
        // 2.1.0
        if (version_compare($this->old_version, '2.1.0', '<') && $this->_columnExists('docman_documents', 'access'))
        {
            $this->_executeQuery("ALTER TABLE `#__docman_documents` CHANGE `access` `access` int(11) NOT NULL default 0");
            $this->_executeQuery("UPDATE `#__docman_documents` SET `access` = 0 WHERE `access` = -1");
        }
        ...
        {
            $this->_executeQuery("ALTER TABLE `#__docman_scans` ADD COLUMN `sent_on` datetime DEFAULT NULL");
    ...
    protected function _removeOldScheduler()
    {
        ...
        if ($extension_id)
        {
            ...
            $i = new JInstaller();
            if (method_exists($i, 'setDatabase')) {
                $i->setDatabase(JFactory::getDbo());
            }
    ...
    protected function _updateRedirectPlugin($installer)
    {
        ...
        if ($plugin_exists)
        {
            ...
            if (method_exists($instance, 'setDatabase')) {
                $instance->setDatabase(JFactory::getDbo());
            }
    ...
    protected function _createFilesContainer()
    {
        ...
        if ($entity->isNew())
        {
            ...
            if (!extension_loaded('gd'))
            {
                ...
                \Joomla\CMS\Factory::getApplication()->enqueueMessage($translator->translate('Your server does not have the necessary GD image library for thumbnails.'));
            }
        ...
        if (!$entity->isNew() && $entity->path)
        {
            ...
            if (!JFolder::exists($path))
            {
                if (!JFolder::create($path)) {
                    \Joomla\CMS\Factory::getApplication()->enqueueMessage(JText::_('Document path cannot be automatically created. Please create the folder structure joomlatools-files/docman-files in your site root.'), 'error');
                }
            }
    ...
    protected function _migrateMenuItems()
    {
        ...
        foreach ($items as $item)
        {
            ...
            if ($query['view'] === 'filteredlist')
            {
                ...
                if (count($q)) {
                    $item->link .= str_replace(array('%5B', '%5D'), array('[', ']'), '&'.http_build_query($q, '', '&'));
                }
