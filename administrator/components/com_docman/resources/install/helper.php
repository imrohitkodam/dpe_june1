...
class JoomlatoolsInstallerHelper
{
    ...
    public function postflight($type, $installer)
    {
        ...
        if ($manifest->dependencies)
        {
            ...
            foreach ($manifest->dependencies->dependency as $dependency)
            {
                ...
                if (method_exists($dependency_installer, 'setDatabase')) {
                    $dependency_installer->setDatabase(JFactory::getDbo());
                }
                ...
                if ((string)$dependency['type'] === 'plugin' && $type != 'update')
                {
    ...
    protected function _discoverExtensions()
    {
        ...
        if (method_exists($installer, 'loadAllAdapters')) {
    ...
    protected function _backupTable($table)
    {
        if ($this->_tableExists($table))
        {
            ...
            $return = JFactory::getDbo()->setQuery(sprintf('RENAME TABLE `%1$s` TO `%2$s`;', $table, $destination))->execute();
        }
        else $return = true;
    ...
    protected function _checkDatabasePrivileges($privileges)
    {
        ...
        $db_name = \Joomla\CMS\Factory::getConfig()->get('db');
