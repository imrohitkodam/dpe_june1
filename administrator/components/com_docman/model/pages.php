...
class ComDocmanModelPages extends KModelAbstract
{
    ...
    protected function _getPages()
    {
        if (!self::$_pages)
        {
            ...
            $items = JApplicationCms::getInstance('site')->getMenu()->getItems($attributes, $values);
    ...
    /**
     * Filters finder compatible pages by removing those that do not play nice with it
     *
     * @param array  $pages Page list
     */
    protected function _filterFinderPages($pages)
    {
        $filtered = array();

        foreach ($pages as $page) {
            if (self::isFinderPage($page)) $filtered[] = $page;
        }

        return $filtered;
    }

    public static function isFinderPage($page)
    {
        $result = false;
        
        // Own filtered pages are to be removed (finder only knows to filter against access, that's it)
    ...
    protected function _actionFetch(KModelContext $context)
    {
        ...
        if ($state->finder) {
            $pages = $this->_filterFinderPages($pages);
        }

        foreach ($pages as &$page) {
            $p = get_object_vars($page);
            $p['params'] = $page->getParams();
        ...
        if ($state->sort && count($pages))
        {
            ...
            if (isset($page[$state->sort]))
            {
                ...
                usort($pages, function($a, $b) use($sort, $direction)
                {
                    $numeric = is_numeric($a[$sort]) && is_numeric($b[$sort]);
