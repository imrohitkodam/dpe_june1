...
class ComDocmanModelDocuments extends ComDocmanModelAbstract
{
    public function __construct(KObjectConfig $config)
    {
        ...
        $this->getState()
            ...
            ->insert('category_access', 'boolean', false)
            ...
            ->insert('search_fields', 'boolean', false)
            ->insert('image', 'com:files.filter.path')
            ->insert('day_range', 'int')
            ->insert('csort', 'cmd') // Category sort
            ->insert('cdirection', 'word', 'asc'); // Category sort direction
    }
    ...
    protected function _initialize(KObjectConfig $config)
    {
        $config->append(array(
            'behaviors' => array(
                ...
                'com://admin/docman.model.behavior.searchable' => ['columns' => ['title', 'description', 'contents.contents', 'cf.custom-fields']],
                'com://admin/docman.model.behavior.pathable' => array('id_column' => 'docman_category_id'),
            )
        ));
    ...
    protected function _addPageFiltersToQuery($page_filters, $query, $callback)
    {
        ...
        /** @var integer|Traversable $filter */
        foreach ($page_filters as $page => $filter)
        {
            ...
            if (is_scalar($filter) && ($filter & static::PAGE_FILTER_ALL_DOCUMENTS)) {
                ...
                foreach ($filter as $type => $value)
                {
                    ...
                    {
                        $x   = 0;
                        $sql = array_map(function ($v) use (&$x, $query) {
                            $sql = "menu_tag.slug = :tag_slug{$x}";
                            $query->bind(array("tag_slug$x" => $v));
                            $x++;
                            return $sql;
                        }, $value);

                        $operator = $this->getConfig()->tag_filter_operator ? strtoupper($this->getConfig()->tag_filter_operator) : 'OR';
                        $parts[]   = '(' . implode(" {$operator} ", $sql) . ')';
                        $join_tags = true;
                    }
    ...
    protected function _buildQueryWhere(KDatabaseQueryInterface $query)
    {
        ...
        if ($state->status === 'published')
        {
            ...
            $query->where(sprintf('(%s (tbl.publish_on IS NULL OR tbl.publish_on <= :publish_date))', $user_status_clause))
              	  ->where(sprintf('(%s (tbl.unpublish_on IS NULL OR tbl.unpublish_on >= :publish_date))', $user_status_clause))
            	  ->bind(array(
                      'publish_date' => $now,
                      'current_user' => $state->current_user
                  ));
        ...
        elseif ($state->status === 'pending')
        {
            ...
            $query->where('(tbl.publish_on IS NOT NULL AND tbl.publish_on >= :publish_date)')
        ...
        elseif ($state->status === 'expired')
        {
            ...
            $query->where('(tbl.unpublish_on IS NOT NULL AND tbl.unpublish_on <= :publish_date)')
	            ->bind(array('publish_date' => $now));
        }
        ...
        if ($state->access || $state->group)
        {
            ...
            else $access = array_merge($access, $this->getGroupLevels($access));

            // Check user's permission to access a category
            if ($state->category_access) {
    ...
    protected function _beforeFetch(KModelContextInterface $context)
    {
        ...
        if (!$state->isUnique() && $csort = $state->csort)
        {
            // Category sorting

            $cdirection = $state->cdirection ?? 'asc';

            $column = sprintf('c.%s', $csort);

            $query->order($column, $cdirection);
        }
    }
}
