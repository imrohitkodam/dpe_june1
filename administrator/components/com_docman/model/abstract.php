...
abstract class ComDocmanModelAbstract extends KModelDatabase
{
    ...
    public function getPagesModel()
    {
        if (!self::$_pages_model) {
            ...
            if (\Joomla\CMS\Factory::getApplication()->isClient('administrator') || $view === 'doclink' || $view === 'documents') {
     ...
     * @return array
     */
    protected function _getPages($filter = 'all')
    {
        if (!isset(self::$_page_cache[$filter]))
        {
            $model = $this->getPagesModel()->reset()->view(array('list', 'flat', 'document', 'tree', 'sitemap', 'search'));

            if ($filter == 'finder') {
                $model->finder(true);
            } else {
                $model->finder(false); // Make sure to disable finder filtering on the already existing page model (reset call above will not reset the state)
            }

            self::$_page_cache[$filter] = $model->fetch();
        }

        return self::$_page_cache[$filter];
    }
    ...
    protected function _getPageFilters($pages)
    {
        ...
        foreach ($pages as $id)
        {
            ...
            if ($page->query['view'] === 'list' || $page->query['view'] === 'tree')
            {
                ...
                if (!empty($page->query['own_categories'])) {
                    $category_owner = $this->getObject('user')->getId();
                }

                // If we have a category view to the root category everything is reachable
                if (empty($created_by) && $created_by !== 0 && $created_by !== '0' && empty($category_owner) && $category_owner !== 0 && $category_owner !== '0' && empty($page->query['slug']) && empty($page->query['tag']))
                {
            ...
            if ($page->query['view'] === 'flat')
            {
                ...
                if (empty($created_by) && $created_by !== 0 && $created_by !== '0' && empty($category_owner) && $category_owner !== 0 && $category_owner !== '0' && empty($page->query['category']) && empty($page->query['tag']))
                {
                    $conditions[$page->id] = static::PAGE_FILTER_ALL_DOCUMENTS | static::PAGE_FILTER_ALL_CATEGORIES;
                    continue;
                }
    ...
    protected function _afterReset(KModelContextInterface $context)
    {
        ...
        if (in_array('page', $modified))
        {
            ...
            {
                $state->page = array_keys($this->_getPages('finder')->toArray());

                $this->_page_filters = $this->_getPageFilters($state->page);
            }
            else
            {
                if ($state->page == 'all') {
