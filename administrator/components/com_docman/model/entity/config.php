...
class ComDocmanModelEntityConfig extends KModelEntityAbstract implements KObjectMultiton
{
    ...
    public function cleanCache()
    {
        ...
        // Clean config view cache
        JCache::getInstance('output')->clean('_system');
    }
    ...
    public function load()
    {
        ...
        $this->doclink_state = JPluginHelper::isEnabled('content', 'doclink');
    }
    ...
    public function save()
    {
        ...
        $this->_setDocLinkStatus($this->doclink_state);
...
<system.webServer>
    ...
    /**
     * DOClink plugin parameters
     * Backward compatibility for doclink embedded player created prior v5.x
     * 
     * @param $param
     * @return mixed
     */
    public function getDoclinkParams($param)
    {
        $plugin = JPluginHelper::getPlugin('content', 'doclink');
    ...
    protected function _setDocLinkStatus($state)
    {
        $state = (int) $state;

        $sql = "UPDATE #__extensions SET enabled = {$state} WHERE name = 'plg_content_doclink'";
