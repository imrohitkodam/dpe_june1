...
class ComDocmanModelNodes extends KModelDatabase
{
    ...
    {
        /** @var KDatabaseQueryUnion $union */
        $union          = KObjectManager::getInstance()->getObject('database.query.union');
        $select_files   = KObjectManager::getInstance()->getObject('database.query.select');
        $select_folders = KObjectManager::getInstance()->getObject('database.query.select');

        $columns = ['*', 'path' => 'CONCAT_WS("/", NULLIF(folder, ""), name)', 'type' => ":type"];

        $select_files->table('docman_files')->columns($columns)->bind(['type' => 'file']);
        $select_folders->table('docman_folders')->columns($columns)->bind(['type' => 'folder']);
    ...
    protected function _buildQueryColumns(KDatabaseQueryInterface $query)
    {
        ...
        // docman_nodes has these as a calculated column in the UNION query
        if ($this->getTable()->getName() !== 'docman_nodes') {
