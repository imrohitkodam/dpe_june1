...
class ComDocmanTemplateHelperString extends KTemplateHelperAbstract
{
    ...
    public function state($config = array())
    {
        ...
        $states = array(
            0 => $translator->translate('Unpublished'),
            1 => $translator->translate('Published')
        );
        ...
        return $states[$config->state];
    }

    /**
     * Returns the author's alias if available otherwise its Joomla user's name
     *
     * @param array $config
     * @return string
     */
    public function author($config = array())
    {
        $config = new KObjectConfigJson($config);
        $config->append(array(
            'entity' => null
        ...
        return $config->entity->created_by_alias ? $config->entity->created_by_alias : $config->entity->getAuthor()->getName();
    }
}
