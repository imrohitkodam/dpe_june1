...
class ComDocmanTemplateHelperListbox extends ComKoowaTemplateHelperListbox
{
    ...
    public function day_range($config = array())
    {
        ...
        $config->append(array(
            ...
            'attribs' => array('class' => array('k-js-scopebar-clearable'))
        ))->append(array(
    ...
    public function documents($config = array())
    {
        ...
        $config->append(array(
            ...
            'label'         => 'title',
            'category_path' => true,
            'fields'        => 'slug,title,category_path'
        ));

        $html = $this->_render($config);

        if ($config->category_path)
        {
            // Override the config to modify the result template and keeping the existing options at the same time
            $html .= '
                <script>
                    kQuery(function($){
                        ...
                        var config = $select.data("select2").options.options;
                        
                        config.templateResult = function(state){
                            var path = state.category_path !== undefined ? state.category_path : "";
                            return state.text + "<div><small>" + path + "</small></div>";
                        }
                        
                        config.templateSelection = function(state){
                            var path = state.category_path !== undefined ? state.category_path : "";
                            return state.text + " <small>" + path + "</small>";
                        }
     ...
     * @param boolean $hierarchy Excludes subcategories with unpublished parents and transforms path to breadcrumbs format
     *
     * @return array $options
     */
    public function fetchCategories($config, $map, $filter = null, $hierarchy = false)
    {
        ...
        else
        {
            ...
            while ($offset < $count)
            {
                ...
                foreach ($entities as $entity)
                {
                    // Get the category IDs for further processing
                    $ids[] = $entity->id;
                ...
                if ($hierarchy)
                {
                    // Exclude unpublished categories including their sub-categories
                    $categories = $this->getObject('com:docman.model.categories')->getHierachicalCategories($ids);
                    ...
                    foreach ($entities as $entity)
                    {
                        if (in_array($entity->id, $categories))
                        {
                            if (is_callable($filter) && !call_user_func_array($filter, array($entity, $config, $entities))) {
                                continue;
                            }

                            // Only invoke the callback function for published categories
                            call_user_func_array($map, array(&$data, $entity, $config, $entities));
                        }
                    }
                }
  ...
  /**
     * Create expiration notice listbox
     *
     * @param array|KObjectConfig $config
     * @return string
     */
    public function notice($config = array())
    {
        $options = array();
        $options[] = $this->option(array('label' => '60-day', 'value' => 60));
        $options[] = $this->option(array('label' => '90-day', 'value' => 90));
        $options[] = $this->option(array('label' => '180-day', 'value' => 180));
    ...
    /**
     * File update mode listbox
     *
     * @param array|KObjectConfig $config
     * @return string
     */
    public function update_mode($config = array())
    {
        $translator = $this->getObject('translator');

        $options = array();
        $options[] = $this->option(array('label' => $translator->translate('Overwrite file'), 'value' => 'overwrite'));
        $options[] = $this->option(array('label' => $translator->translate('Prompt user'), 'value' => 'confirm'));
        $options[] = $this->option(array('label' => $translator->translate('Create new version'), 'value' => 'new'));
