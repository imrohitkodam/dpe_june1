...
class ComDocmanViewConfigHtml extends ComDocmanViewHtml
{
     ...
     * CSV export fields
     *
     * @var array|KObjectConfigInterface|mixed
     */
    protected $_export_fields = array();

    public function __construct(KObjectConfig $config)
    {
        parent::__construct($config);

        $this->_export_fields = KObjectConfig::unbox($config->export_fields);
    }
    ...
    protected function _fetchData(KViewContext $context)
    {
        ...
        $this->_exportFields($context);
    }

    /**
     * Build CSV export fields
     *
     * @param KViewContext $context
     * @return void
     */
    protected function _exportFields(KViewContext $context)
    {
        $export_fields  = $this->_export_fields;
        $current_fields = $context->data->config->csv_fields;
        $fields = array();

        // Fetch export field names
        if ($current_fields)
        {
        ...
        // Check if export fields need to be rebuilt
        // e.g. new field(s) are being added via /joomlatools-config/docman.php
        $sort_current = $fields;
        $sort_export = $export_fields;
        sort($sort_current);
        sort($sort_export);

        // Rebuild the export fields
        if ($sort_current != $sort_export)
        {
            $current_fields = array();

            foreach ($export_fields as $property)
            {
                $current_fields[] = array(
                    $property => 1
                );
            ...
            $context->data->config->csv_fields = $current_fields;
        }
    }
}
