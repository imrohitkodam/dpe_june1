			...
			foreach ($fields as $f)
			{
				...
				if (in_array($f->id, $conditionalFields))
				{
					...
					foreach ($conditions as $condition)
					{
						...
						{
							$matchCase = ($conditionMatch == 1) ? "[OR]" : "[AND]" ;
						}
						...
						{		
							...
							if ($fieldTable->type == 'checkbox' && empty($optionValue))
							{
								$optionValue = 1;
							}
				...
				if (!empty($f->params))
				{
					...
					foreach ($fieldAttribute as $attribute => $fieldparam)
					{
						...
						{ 
							...
							// DPE Hack to show multiselect attribute values comma seperated 
							$fieldparam = is_object($fieldparam)? json_decode( json_encode($fieldparam), true):$fieldparam; // After Test php8.1

							if ($attribute == 'tjfields')
							{
								$fieldparam = json_encode($fieldparam);
							}
							elseif (is_array($fieldparam))
							{	
								$fieldparam = implode(',', $fieldparam);
							}
							// DPE Hack end

							// This is temporary solution for the dpechecklist . need to change this .
							if ($f->type != 'dpechecklist')
							{
				...
				if (isset($f->extra_options))
				{
					...
					if ($f->type == 'list' || $f->type == 'tjlist')
					{
						...
						// DPE Hack -  Removed isset($fieldAttribute->multiple) for single select type field
						if ($fieldAttribute->multiple != 'true' && !$fieldAttribute->multiple)
						{
							// DPE Hack - Will get merged into core soon - Set Default blank Option
