...
class TjfieldsModelField extends AdminModel
{
	...
	public function getItem($pk = null)
	{
		...
		if ($item = parent::getItem($pk))
		{
			...
			if ($input->get('id', '', 'INT'))
			{
				// opt.feedback to get the feedback value.
				$db = Factory::getDbo();
				$query = $db->getQuery(true);
				$query->select('opt.id as optionid,opt.options as name,opt.value, opt.feedback FROM #__tjfields_options as opt');
			...
			//DPE Hack
			if($item->type =='radio' || $item->type =='tjlist')
			{ 

				Table::addIncludePath(JPATH_ROOT . '/administrator/components/com_dpe/tables');
  	     		$notificationConfigTable = Table::getInstance('NotificationConfig', 'DpeTable');
  	     		$notificationConfigTable->load(array('ucm_field_id' => $item->id));

  	     		if($notificationConfigTable->id)
  	     		{
  	     			$item->show_notification = $notificationConfigTable->show_notification;
  	     		}
			}
			
			// DPE Hack: expose showFeedbackOnForm explicitly since it was moved outside the params fieldset in XML
			if (!empty($item->params))
			{
				if (is_array($item->params) && isset($item->params['showFeedbackOnForm']))
				{
					$item->showFeedbackOnForm = $item->params['showFeedbackOnForm'];
				}
				elseif (is_string($item->params))
				{
					$decodedParams = json_decode($item->params);
					if (isset($decodedParams->showFeedbackOnForm))
					{
						$item->showFeedbackOnForm = $decodedParams->showFeedbackOnForm;
					}
				}
			}
	...
	public function save($data)
	{
		...
		if (empty($data['showFeedback']))
		{
			$data['showFeedback'] = 0;
		}
		// DPE Hack start
		if (($data['showFeedbackOnForm'] == 0) || ($data['showFeedbackOnForm'] == ''))
		{
			$data['params']['showFeedbackOnForm'] = 0;
		}else{
			$data['params']['showFeedbackOnForm'] = 1;
		}
		// DPE Hack End
		...
		if ($data['type'] == "radio" || $data['type'] == "single_select" || $data['type'] == "multi_select" || $data['type'] == "tjlist" || $data['type'] == "list")
		{
			$data['saveOption'] = 1;
		}
		...
		// DPE Hack - Normalize parameter structure for dpechecklist
		if ($data['type'] == 'dpechecklist' && isset($jform['params']))
		{
			$normalizedTjFields = array();

			foreach ($jform['params'] as $key => $paramData)
			{
				if (strpos($key, 'tjfields[') === 0 && is_array($paramData))
				{
					// Extract index from key like "tjfields[0" or "tjfields[0]"
					preg_match('/tjfields\[(\d+)/', $key, $matches);
					$index = isset($matches[1]) ? (int)$matches[1] : null;

					if ($index !== null)
					{
						$normalizedTjFields[$index] = $paramData;
					}
					
					// Remove the malformed key from data to be saved
			...
			// Add the normalized array to params if we found any fields
			if (!empty($normalizedTjFields))
			{
				ksort($normalizedTjFields); // Ensure indices are in order
				$data['params']['tjfields'] = $normalizedTjFields;
			}
		}
		// DPE Hack end
		...
		// DPE Hack to store class for radio

		if (!empty($data['validation_class'])|| $data['validation_class'] == trim($data['validation_class']) || str_contains($data['validation_class'], ' ')) 
		{
		   $classes = explode(" ", $data['validation_class']);

		   foreach($classes as $key => $class)
		   {
			   	if ($class == 'btn-group' && $data['type'] == 'radio')
			   	{
			   		$classes[$key] = 'tj_btn_group';
			   	}
		   }

		    $validClass = implode(' ', $classes);
		   $data['validation_class'] = $validClass;
		}
		// Hack end	
		...
		if ($table->save($data) === true)
		{
				...
				// DPE Hack to store notification configuration

				if ($data['type'] == 'radio' || $data['type'] == 'tjlist' && isset($data['showNotificationConfiguration']))
				{
					
					Table::addIncludePath(JPATH_ROOT . '/administrator/components/com_dpe/tables');
  	     		    $notificationConfigTable = Table::getInstance('NotificationConfig', 'DpeTable');
  	     			$notificationConfigTable->load(array('ucm_field_id' => $id));

  	     			$notificationConfigTable->ucm_field_id       = $id;
  	     			$notificationConfigTable->show_notification = $data['showNotificationConfiguration'];
 	     		   	$notificationConfigTable->store();	
  	    		}
		}

		// Check if name feild is unique
		$is_unique = $TjfieldsHelper->checkIfUniqueName($data['name']);		echo $id;
			...
			if ($data['saveOption'] == 1)
			{
				...
				else
				{
					...
					foreach ($options as $option)
					{
						...
						// This is for feedback column .
						$obj->feedback = $option['feedback'];
						$obj->field_id = $id;
						$obj->ordering = $order;
						$order++;
	...
	public function getRelatedFieldOptions($fieldId)
	{
		...
		{ 
			...
			if ($fieldParams->get('clusterAware') == 1 && !($fieldParams->get('genericschool')))
			{
				$clusterId = $jInput->get("cluster_id", 0, "INT");

				if(!$clusterId)
				{
					$clusterId = $jInput->get("clusterId", 0, "INT");
				...
				elseif (!Factory::getUser()->authorise('core.manageall', 'com_cluster'))
				{
					...
					// DPE Hack - For School Admin check

					$filter_process = $jInput->get('filter_process', 'myprocess', 'STRING');
					
					if ($filter_process == 'generic')
					{
						$dpeParam         = JComponentHelper::getParams('com_dpe');

						if ($dpeParam->get('cluster_id', '0', 'INT') > 0)
						{
							$query->where($db->quoteName('cluster_id') . ' = ' . (int) $dpeParam->get('cluster_id', '0', 'INT'));
						}
					}
					else
					{
						if (($key = array_search('all', $usersClusters)) !== false)
						{
							unset($usersClusters[$key]);
						}
			...
			// DPE Hack for generic school data

			if ($fieldParams->get('genericschool') == 1 && !($fieldParams->get('clusterAware'))) 
			{
				JLoader::import("/components/com_dpe/includes/dpe", JPATH_SITE);

				$params = DPE::config();

				$genericSchoolId = (int) $params->get('cluster_id', '0');

				$query->where($db->quoteName('cluster_id') . ' IN (' .  $genericSchoolId . ')');
			}
			...
			if(!$parentContentId){
				$parentContentId = $jInput->get('content_id', 0, 'INT');
			}
