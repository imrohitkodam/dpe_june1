...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Get all buttons that have '_jform_' in their ID
    document.querySelectorAll('button[id*="jform_request_id"]').forEach(function(button) {
        button.addEventListener('click', function(e) {
            e.preventDefault();

            var matches = button.getAttribute('data-bs-target');
            var url = jQuery(matches).attr('data-url');
            var modalId = matches.replace(/^#/, '');
            ...
            // Load URL into modal body as iframe
            var modalBody = modalElement.querySelector('.modal-body');
            modalBody.innerHTML = '<iframe class="iframe" src="' + url + '" style="width:100%; height:400px; border:none;"></iframe>';

            // Open the Joomla modal
            var modal = new bootstrap.Modal(modalElement);
            modal.show();

            // Once iframe is loaded
            modalBody.querySelector('iframe').onload = function() {
                var iframeWindow = this.contentWindow;
                var iframeDoc = this.contentDocument || iframeWindow.document;

                // ===  Handle existing onclick attributes ===
                iframeDoc.querySelectorAll('a[onclick]').forEach(function(a) {
                    a.addEventListener('click', function(ev) {
                        ev.preventDefault();
                        var onclickAttr = a.getAttribute('onclick');
                        if (!onclickAttr) return;

                        iframeWindow.parent.eval(onclickAttr); // safely call original parent function
                        modal.hide();
                        document.querySelectorAll('.modal-backdrop').forEach(b => b.remove());
                    });
                });

                // ===  Handle links with data-function, data-id, data-title ===
                iframeDoc.querySelectorAll('a.select-link[data-function]').forEach(function(a) {
                    a.addEventListener('click', function(ev) {
                        ev.preventDefault();

                        var funcName = a.getAttribute('data-function');
                        var id = a.getAttribute('data-id');
                        var title = a.getAttribute('data-title') || '';

                        // Call the parent Joomla function dynamically
                        if (window.parent && typeof window.parent[funcName] === 'function') {
                            window.parent[funcName](id, title, null, null);
                        }
