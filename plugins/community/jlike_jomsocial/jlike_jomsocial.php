...
class plgCommunityJlike_jomsocial extends JPlugin {
	...
	public function onBeforeDisplaylike()
	{
		...
		{
			case 'profile' :
				$cont_id=JRequest::getInt('userid');
				$columnName = 'username';
				$dbTable = '#__users';
			...
			case 'video':
				$cont_id=JRequest::getInt('videoid');
			...
			case 'album':
				...
				$cont_id=$photoid=JRequest::getInt('photoid');
			...
			case 'viewgroup':
			case 'viewmembers':
				$cont_id=$groupid=JRequest::getInt('groupid');
				$title = 'this group';
				$columnName = 'name';
			...
			case 'viewevent':
			case 'viewguest':
				$cont_id=$eventid=JRequest::getInt('eventid');
		...
		JRequest::setVar ( 'data', json_encode ( array ('cont_id'=>$cont_id,'element' => $jlikeelement, 'title' => $title, 'url' => $route,'plg_name'=>'jlike_jomsocial','show_comments'=>$show_comments, 'show_like_buttons'=>$show_like_buttons) ) );
