...
class PlgContentJLike_Articles extends CMSPlugin
{
	...
	public function SetValues($context, $article, $params, $page, $show_like_buttons, $show_comments, $show_recommend, $showassignbtn)
	{
		...
		/** @scrutinizer ignore-deprecated */ JRequest::setVar('data',
			json_encode(
				array(
					'cont_id' => $cont_id,
					'element' => $context,
	 ...
	 * @since  1.0.0
	 */
	public function getjlike_articlesOwnerDetails($cont_id)
	{
	...
	public function getjlike_articlesLikesNotificationDetails($extraParams, $data)
	{
		...
		$link = JUri::root() . substr(JRoute::_($data['url']), strlen(JUri::base(true)) + 1);
		$link = '<a href="' . $link . '">' . $data['title'] . '</a>';

		$owner_id = $this->getjlike_articlesOwnerDetails($data['element_id']);
	...
	public function getjlike_articlesCommentNotificationDetails($extraParams, $annotation_id, $commentData)
	{
		...
		$link = JUri::root() . substr(JRoute::_($extraParams['url']), strlen(JUri::base(true)) + 1);
		$link = '<a href="' . $link . '">' . $extraParams['title'] . '</a>';

		$owner_id = $this->getjlike_articlesOwnerDetails($extraParams['cont_id']);
	 ...
	 * @since  1.0.0
	 */
	public function jlike_articlesGetSocialIntegration()
	{
	 ...
	 * @since  1.0.0
	 */
	public function jlike_articlesGetElementData($article_id)
	{
		$data = array();
