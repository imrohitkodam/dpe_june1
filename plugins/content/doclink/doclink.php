...
class PlgContentDoclink extends CMSPlugin
{
    /**
    ...
    protected function _enrichDocumentLinks(&$links, &$text)
    {
        ...
        foreach ($links as &$link)
        {
            ...
            // New 3.6 tag overrides player configuration
            $insert = stristr($link->attributes['class'], 'doclink-insert');
            $embed  = stristr($link->attributes['class'], 'doclink-embed');

            if ($embed)
            {
                if ($viewer =  $this->_renderDocument($link, $document)) {
                    $this->_replaceLink($link, $viewer, $text);
                }
            ...
            if (!$insert && $this->_getConfig()->doclink_show_player && $document->isPlayable()) {
                continue;
            }
            ...
            {
                $menu = $application->getMenu()->getItem($link->query['Itemid']);

                if ($menu)
                {
                    if ($menu->getParams()->get('document_title_link') === 'download' && $menu->getParams()->get('track_downloads'))
                    {
                    ...
                    if ($menu->getParams()->get('download_in_blank_page')) {
    ...
    protected function _replaceLinksWithPlayers(&$links, &$text)
    {
        ...
        foreach ($links as &$link)
        {
            ...
            // New 3.6 tags override player configuration
            $embed      = (bool)stristr($link->attributes['class'], 'doclink-embed');
            $insert     = (bool)stristr($link->attributes['class'], 'doclink-insert');
            $can_render = $insert || $embed ? $embed : $this->_getConfig()->doclink_show_player;
            ...
            $helper = $this->getObject('com://site/docman.template.helper.player', [
                'template' => $this->getObject('com:docman.view.html')->getTemplate()
            ]);

            $player = $helper->render(array('document' => $document, 'asset' => true));
    ...
    protected function _enrichLegacyLinks(&$links, &$text)
    {
        ...
        if (count($document_ids))
        {
            ...
            foreach ($links as &$link)
            {
                ...
                // New v3.6 tag overrides player configuration
                if (stristr($link->attributes['class'], 'doclink-embed'))
                {
                    $this->_replaceLink($link, $this->_renderDocument($link, $document), $text);
                    continue;
                }
    ...
    protected function _renderLink($link, $entity, $parameters = array())
    {
        ...
        if (!$this->_getConfig()->doclink_show_size) {
            $parameters['show_size'] = false;
        }

        if (!$this->_getConfig()->doclink_show_icon) {
    ...
    protected function _renderDocument($link, $entity)
    {
        $result = null;

        if ($entity->isViewable() && stristr($link->attributes['class'], 'doclink-embed')) {
            $result = $this->getObject('com://site/docman.template.helper.viewer')->render(['document' => $entity]);
        }
    ...
    {
        return $this->getObject('com://admin/docman.model.configs')->fetch();
    }
