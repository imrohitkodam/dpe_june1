...
(function (jQuery) {
    ...
    jQuery.contentbuilder = function (element, options) {
        ...
        this.applyBehavior = function () {
            ...
            $element.find('.row-html').off('click').on('click', function () {
                const $textarea = jQuery('#txtCustomHtml');
                const $modal = jQuery('#md-customhtml');
                const $thisRow = jQuery(this).closest('.ui-draggable').children().not('.row-tool');
                let $activeRow = $thisRow;
            
                // Prepare content
                if ($activeRow.data('mode') === 'code') {
                    $textarea.val(decodeURIComponent($activeRow.attr('data-html')));
                } else {
                    jQuery('#temp-contentbuilder').html($activeRow.html());
                    jQuery('#temp-contentbuilder [contenteditable]').removeAttr('contenteditable');
                    jQuery('#temp-contentbuilder *[class=""]').removeAttr('class');
                    jQuery('#temp-contentbuilder *[style=""]').removeAttr('style');
                    jQuery('#temp-contentbuilder .ovl').remove();
                    jQuery('#temp-contentbuilder').find('p').each(function () {
                    ...
                    let html = jQuery('#temp-contentbuilder').html().trim();
                    html = html.replace(/<font/g, '<span').replace(/<\/font/g, '</span');
                    $textarea.val(html);
                }
                ...
                // Save button
                jQuery('#btnCustomHtmlOk').off('click').on('click', function () {
                    let newHtml = $textarea.val();
                    // Ensure anchor links open in new tab if not already specified
                    newHtml = newHtml.replace(/<a\s+(?![^>]*target=)/gi, '<a target="_blank" ');

                    // Ensure margin divs get contenteditable="false"
                    let $tmp = jQuery('<div>').html(newHtml);
                    $tmp.find('div[style="margin:1em 0 2.5em;"]').attr('contenteditable', 'false');
                    newHtml = $tmp.html();
                    
                    if ($activeRow.data('mode') === 'code') {
                        $activeRow.attr('data-html', encodeURIComponent(newHtml));
                        $activeRow.html('');
                    } else {
                        $activeRow.html(newHtml);
                    }
...
(function (jQuery) {
    ...
    jQuery.contenteditor = function (element, options) {
        ...
        this.init = function () {
            ...
            $element.find(this.settings.editable).attr('contenteditable', true);

            $element.find(this.settings.editable).on('focus', function () {
            ...
            var html_rte = '<div id="rte-toolbar"' + toolbar_attr + '>' +
                        //  Undo/Redo buttons
                        '<a href="#" data-rte-cmd="undo"> <i class="fa fa-undo"></i> </a>' +
                        '<a href="#" data-rte-cmd="redo"> <i class="fa fa-redo"></i></a>' +
                ...
                '<div class="md-modal" id="md-createlink">' +
			        '<div class="md-content">' +
				        '<div class="md-body">' +
                            ...
                            '<input type="text" id="txtLink" class="inptxt" style="float:left;width:50%;" value="http:/' +
                            '<br style="clear:both">' +
            ...
            $(document).on('paste', '[contenteditable="true"], textarea, input', function(e) {
                e.preventDefault();
                ...
                if (html && document.queryCommandSupported("insertHTML")) {
        ...
        this.render = function () {
            ...
            // Function is used for Youtub vidoe URL adding some emebd 
            function getYoutubeEmbedURL(inputUrl) {
                const youRegex = /(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:watch\?v=|embed\/)|youtu\.be\/)([\w\-]{11})/;
                const match = inputUrl.match(youRegex);
            ...
            let selectedIframe = null; // Global to track selected iframe
            
            $element.find(".embed-responsive").off('mouseenter mouseleave').hover(function () {
                if (jQuery(this).parents("[data-mode='code']").length > 0) return;
            
                // Zoom handling
                let zoom = localStorage.zoom;
                if (!zoom || zoom === 'normal' || zoom === 'NaN') zoom = 1;
                if (typeof zoom === 'string' && zoom.indexOf('%') !== -1) {
                    zoom = parseFloat(zoom.replace('%', '')) / 100;
                }
                ...
                const topPos = ((offset.top - scrollTop) + (height / 2)) * zoom;
                const leftPos = ((offset.left - scrollLeft) + (width / 2)) * zoom;
            
                selectedIframe = $target.children('iframe').first(); // Save the current iframe
                const $tool = jQuery("#divFrameLink");
                if (!$tool.parent().is("body")) {
                    jQuery("body").append($tool); // Prevent being clipped by container
                }
                ...
                requestAnimationFrame(() => {
                    ...
                    const safeTop = Math.max(10, Math.min(topPos, windowHeight - 10));
                    const safeLeft = Math.max(10, Math.min(leftPos, windowWidth - 10));
                ...
                // Bind click on floating edit button
                $tool.off('click').on('click', function () {
                    if (!selectedIframe) return;

                    // Store reference in the modal
                    jQuery('#md-youtubsrc').data('iframe', selectedIframe);
            
                    const oldsrc = selectedIframe.attr('src');
            
                    // Remove any old modals
                    jQuery('#md-createsrc, .md-overlay').remove();
            
                    // Show your custom modal
                    jQuery('#md-youtubsrc').css({
                        display: 'block',
                        opacity: '1',
                        visibility: 'visible',
                        zIndex: '99999'
                    ...
                    jQuery('#youtublink').val(oldsrc); // Fill old link
                });
            
                // Prevent flicker when hovering on the button itself
                $tool.hover(
                    function () { jQuery(this).stop(true, true).show(); },
                    function () { jQuery(this).stop(true, true).fadeOut(0); }
                );
            ...
            //  Handle modal OK button to update iframe src
            jQuery(document).on('click', '#btnSrcOk', function () {
                const userInput = jQuery('#youtublink').val().trim();
                const embedUrl = getYoutubeEmbedURL(userInput);

                 // Retrieve iframe reference from modal
                const $iframe = jQuery('#md-youtubsrc').data('iframe');
            
                if (embedUrl) {
                    if ($iframe && $iframe.length) {
                        $iframe.attr('src', embedUrl);  // Update only that iframe
                    } else {
                        alert('No iframe selected!');
                    }
                } else {
                    alert('Invalid YouTube link');
                }
            
                jQuery('#md-youtubsrc').hide();
            });
            ...
            $element.hover(function (e) {
                ...
                if(cb_edit && !fixedimage){
                   // Get image dimensions and position
                    var imgOffset = $imgActive.offset();
                    var imgWidth = $imgActive.outerWidth();
                    var imgHeight = $imgActive.outerHeight();

                    // Get tool dimensions
                    var toolWidth = jQuery("#divToolImg").outerWidth();
                    var toolHeight = jQuery("#divToolImg").outerHeight();
                    var settingsWidth = jQuery("#divToolImgSettings").outerWidth();
                    var settingsHeight = jQuery("#divToolImgSettings").outerHeight();

                    // Center the toolbar on the image
                    var topCenter = imgOffset.top + (imgHeight / 2) - (toolHeight / 2);
                    var leftCenter = imgOffset.left + (imgWidth / 2) - (toolWidth / 2);

                    // Optionally offset divToolImgSettings a bit lower (like _top2 logic)
                    var topSettings = topCenter + 35; // You can adjust this value

                    // Position main image toolbar
                    jQuery("#divToolImg").css({
                        top: topCenter + "px",
                        left: leftCenter + "px",
                        display: 'none'
                    }).fadeIn(20);

                    // Position settings toolbar (aligned to center by default, shifted down)
                    jQuery("#divToolImgSettings").css({
                        top: topSettings + "px",
                        left: leftCenter + "px",
                        display: 'none'
                    }).fadeIn(20);
