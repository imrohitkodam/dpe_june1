...
class TjqueueCoreCopyitem
{
	...
	public function consume($message)
	{
		...
		//~ $logData = "logData".print_r($model, true);
		//~ file_put_contents('/var/www/ttpl-rt-30-php8.local/public/dpej41/test.log', $logData, FILE_APPEND);

		//~ echo "targetClient".$targetClient;
		//~ die;
		...
		foreach ($extraFieldsData as $fieldKey => $fieldValue)
		{
				...
				foreach ($subFormData as $keyData => $data)
				{
					...
					foreach ((array) $data as $key => $d)
					{
						...
						elseif($fieldTable->type == 'file' || $fieldTable->type == 'image')
						{
							...
							$subformFileData['user_id'] = $userId;
							$data[$subFieldName] = $subformFileData;
						}
						elseif ($fieldTable->type == 'cluster')
						{
							$data[$subFieldName] = $content->clusterId;
						}
						else
						{
							$data[$subFieldName] = $d;
			...
			else
			{
				...
				{
					$extraFieldsData[$targetFieldName] = $content->clusterId;
				}
				else
				{
		...
		try
		{
			...
			if (isset($recipients))
			{
				...
				// Set body for email
				$mailer->setBody(Text::sprintf('PLG_TJQUEUE_CORE_EMAIL_BODY', Factory::getUser($userId)->name));
