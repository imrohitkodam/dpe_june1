...
class PlgPrivacyJTicketing extends PrivacyPlugin
{
	...
	public function onPrivacyCollectAdminCapabilities()
	{
		...
		return array(
			JText::_('PLG_PRIVACY_JTICKETING') => array(
				JText::_('PLG_PRIVACY_JTICKETING_PRIVACY_CAPABILITY_USER_DETAIL'),
				JText::_('PLG_PRIVACY_JTICKETING_PRIVACY_VENDOR_CAPABILITY_USER_DETAIL'),
				JText::_('PLG_PRIVACY_JTICKETING_PRIVACY_REPORTS_CAPABILITY_USER__DETAIL'),
				JText::_('PLG_PRIVACY_JTICKETING_PRIVACY_JLIKE_CAPABILITY_USER__DETAIL'),
				JText::_('PLG_PRIVACY_JTICKETING_PRIVACY_ACTIVITY_STREAM_CAPABILITY_USER_DETAIL'),
				JText::_('PLG_PRIVACY_JTICKETING_PRIVACY_TJNOTIFICATION_CAPABILITY_USER_DETAIL'),
				JText::_('PLG_PRIVACY_JTICKETING_PRIVACY_HIERARCHY_CAPABILITY_USER_DETAIL')
			)
		);
	}
	...
	public function onPrivacyExportRequest(PrivacyTableRequest $request, JUser $user = null)
	{
		...
		$domains[] = $this->createJTicketingEventDomain($userTable);
	 ...
	 * @since  2.3.4
	 */
	private function createJTicketingUserDomain(JTableUser $user)
	{
	 ...
	 * @since  2.3.4
	 */
	public function createJTicketingVenueDomain(JTableUser $user)
	{
	 ...
	 * @since  2.3.4
	 */
	public function createJTicketingEventDomain(JTableUser $user)
	{
		...
		$query = $this->db->getQuery(true)
			...
			->from($this->db->quoteName('#__jticketing_media_files'))
			->where($this->db->quoteName('created_by') . ' = ' . (int) $user->id);
	 ...
	 * @since  2.3.4
	 */
	public function createJTicketingOrderDomain(JTableUser $user)
	{
	 ...
	 * @since  2.3.4
	 */
	public function createJTicketingAttendeesDomain(JTableUser $user)
	{
	 ...
	 * @since  2.3.4
	 */
	public function createJTicketingWaitListDomain(JTableUser $user)
	{
	 ...
	 * @since  2.3.4
	 */
	public function createJTicketingCouponsDomain(JTableUser $user)
	{
		...
		$query = $this->db->getQuery(true)
			->select(
				...
				value, val_type, limit, max_per_user, description, params,
	...
	public function onPrivacyCanRemoveData(PrivacyTableRequest $request, JUser $user = null)
	{
		...
		if ($this->checkUserIsInWaitList($user) === false)
		{
			$status->canRemove = false;
			$status->reason    = JText::_('PLG_PRIVACY_JTICKETING_ERROR_WAITLIST_CANNOT_REMOVE_USER_DATA');

			return $status;
		}

		// Check requested user is in attendees table
		if ($this->checkUserIsInAttendees($user) === false)
		{
			$status->canRemove = false;
			$status->reason    = JText::_('PLG_PRIVACY_JTICKETING_ERROR_ATTENDEES_CANNOT_REMOVE_USER_DATA');

			return $status;
		}

		// Check requested user has created any venue. And the created venue has used for a event
		$jticketingModelVenues = JModelLegacy::getInstance('venues', 'JticketingModel');
		...
		$jticketingModelEvents = JModelLegacy::getInstance('events', 'JticketingModel');
		...
		else
		{
			...
			$eventName = array();
			$jticketingModelorders = JModelLegacy::getInstance('orders', 'JticketingModel');
	 ...
	 * @since  2.3.4
	 */
	private function checkUserIsInWaitList($user)
	{
		$jticketingTableWaitinglist = JTable::getInstance('waitinglist', 'JTicketingTable', array());
	 ...
	 * @since  2.3.4
	 */
	private function checkUserIsInAttendees($user)
	{
		$jticketingTableAttendees = JTable::getInstance('attendees', 'JTicketingTable', array());
