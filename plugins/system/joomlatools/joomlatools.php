...
class PlgSystemJoomlatools extends CMSPlugin
{
    /**
    ...
    public function __construct($subject, $config = array())
    {
        ...
        {
            if ($offset = Factory::getUser()->getParam('timezone')) {
                Factory::getConfig()->set('offset', $offset);
            }
        }

        //Bugfix: Set display_errors accordingly
        if(Factory::getConfig()->get('error_reporting') == 'none') {
    ...
    public function bootstrap()
    {
        ...
        if (file_exists($path))
        {
            ...
            if (!defined('KOOWA'))
            {
                ...
                $application = Factory::getApplication()->getName();
                ...
                Koowa::getInstance(array(
                    'cache'           => false, //Factory::getConfig()->get('caching')
                    'cache_namespace' => 'koowa-' . $application . '-' . md5(Factory::getConfig()->get('secret')),
                ...
                $bootstrapper = KObjectManager::getInstance()->getObject('object.bootstrapper')
                    ...
                    ->registerApplication('site', JPATH_SITE . '/components', Factory::getApplication()->isClient('site'))
                    ->registerApplication('admin', JPATH_ADMINISTRATOR . '/components', Factory::getApplication()->isClient('administrator'));
            ...
            if (Factory::getConfig()->get('live_site'))
            {
                $request->setBasePath(rtrim(Uri::base(true), '/\\'));
                $request->setBaseUrl($manager->getObject('lib:http.url', array('url' => rtrim(Uri::base(), '/\\'))));
            }

            /**
             * Plugin Bootstrapping
             */
            PluginHelper::importPlugin('koowa', null, true);
    ...
    protected function _proxyEvent($event, $args = array())
    {
        ...
        if (class_exists('Koowa')) {
            $result = Koowa::getObject('event.publisher')->publishEvent($event, $args, Factory::getApplication());
        }
