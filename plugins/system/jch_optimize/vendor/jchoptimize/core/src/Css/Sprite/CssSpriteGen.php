...
//                                        $this->sCss .= " } \n";
	...
	{
		JCH_DEBUG ? Profiler::start('OptimizeSprite') : null;

		$aQuery = array(
			'dir'   => Helper::prepareImageUrl($sFileName),
			'async' => 1,
			'task'  => 'optimize'
		);

		$url = Paths::ajaxUrl('optimizeimages');

		Helper::postAsync($url, $this->params, $aQuery);

		JCH_DEBUG ? Profiler::stop('OptimizeSprite', true) : null;
	}
