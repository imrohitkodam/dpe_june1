...
class PlgSystemJoomlatoolsupdater extends CMSPlugin
{
    ...
    public function onInstallerBeforePackageDownload(&$url, &$headers)
    {
        ...
        if ($error) {
            ...
            if ($app->isClient('administrator') && Factory::getDocument()->getType() === 'html') {
    ...
    protected function _addUpdateNotifier($extension = null)
    {
        ...
        $updates_url      = Uri::base() . 'index.php?option=com_installer&view=update&task=update.find&' . $token;
        $updates_ajax_url = Uri::base() . 'index.php?option=com_installer&view=update&task=update.ajax&eid=0&skip=700&' . $token;
...
JS;
    ...
    public function onAfterInitialise()
    {
        try {
            ...
            if (JDEBUG) Factory::getApplication()->enqueueMessage($e->getMessage(), 'error');
        }
