...
class PlgSystemJoomlatoolsupdaterLicense extends KObject implements KObjectSingleton
{
    ...
    protected function _validateToken($token)
    {
        ...
        if ($site['key'] !== "ffffffff-ffff-ffff-ffff-ffffffffffff" && $site['key'] !== $this->getSiteKey()) {
    ...
    protected function _sendRequest($url, $options = array())
    {
        ...
        $method = isset($options['method']) ? $options['method'] : (isset($options['data']) ? 'POST' : 'GET');
        ...
        try
        {
            if (version_compare(JVERSION, '4', '<')) {
                $http = (new  \Joomla\CMS\Http\HttpFactory)->getHttp();
            } else {
            ...
            switch($method)
            {
                ...
                case 'trace':
                    ...
                    $response = $http->{$method}($url, $headers, $timeout);
                ...
                case 'patch':
                    ...
                    $response = $http->{$method}($url, $data, $headers, $timeout);

                    break;

                case 'delete':

                    $response = $http->{$method}($url, $headers, $timeout, $data);
        ...
        if (isset($status_code) && ($status_code < 200 || $status_code >= 300)
            ...
            throw new \UnexpectedValueException('Problem in the request. Request returned ' . $status_code . ' with response: ' . $body, $status_code);
        }
