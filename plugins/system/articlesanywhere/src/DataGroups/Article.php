...
class Article extends DataGroup
{
    ...
    public function getQueryKeys()
    {
        ...
        if (RL_RegEx::match('^/?(image|video|youtube|vimeo)-(count|random|[0-9]+)(-|$)', $this->key))
        {
            return [
                'article.title',
                'article.introtext',
    ...
    public function getValue()
    {
        ...
        if ($this->key == 'image-count')
        {
            return Image::getImageCountFromText($this->getArticleText());
        }

        $image_regex = '^image-(?<id>random|[0-9]+)$';

        if (RL_RegEx::match($image_regex, $this->key, $match))
        {
            return $this->getContentImageByMatch($match, $this->getArticleText());
        }

        $video_regex = '^(?<closing>/?)(?<platform>video|youtube|vimeo)-(?<id>random|[0-9]+)$';

        if (RL_RegEx::match($video_regex, $this->key, $match))
        {
            return $this->getContentVideoByMatch($match, $this->getArticleText());
        }
    ...
    protected static function getPossibleRegexKeys()
    {
        return [
            ...
            '^image-(?:intro|introtext|fulltext|count|random|[0-9]+)$',
    ...
    private function canEdit()
    {
        ...
        if (RL_User::authorise('core.edit', $asset))
        {
            return true;
        }

        // Now check if edit.own is available.
        if ( ! RL_User::authorise('core.edit.own', $asset))
        {
            return false;
        }
    ...
    private function isCheckedOut()
    {
        ...
        return $checked_out && $checked_out != RL_User::getId();
    }
}
