...
class  plgSystemOfflajnParams extends CMSPlugin
{
  ...
  function onBeforeRender()
  {
    ...
    if ($app->isClient('administrator') && @$_REQUEST['option'] == 'com_installer' && @$_REQUEST['view'] == 'update') {
    ...
    if ($app->isClient('administrator') && @$_REQUEST['option'] == 'com_installer' && @$_REQUEST['task'] == 'update.extra_query') {
      ...
      die('ok');
		...
		if(!$app->isClient('administrator') || !isset(${'_SESSION'}['offlajnurl']) || !isset(${'_SESSION'}['offlajnurl'][$_SERVER['REQUEST_URI']])){
			return;
		}
    //if(version_compare(JVERSION,'3.0.0','ge')) return;
        ...
        $db->setQuery('SELECT template, params FROM #__template_styles WHERE `client_id` = 1 AND `id`= '. (int)$template_style_id.' ORDER BY id ASC');
  		...
  		if(file_exists(JPATH_THEMES.'/'.$row->template)){
    		...
    		$tmpl->params = new Registry($row->params);
  		}
		}else{
