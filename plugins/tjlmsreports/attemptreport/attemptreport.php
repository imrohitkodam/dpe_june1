...
class PlgTjlmsreportsAttemptreport extends JPlugin
{
	/**
	...
	public function __construct(&$subject, $config)
	{
		...
		$extension    = 'plg_tjlmsreports_attemptreport';
		$base_dir     = JPATH_BASE;
		$language_tag = 'en-GB';
		$reload       = true;
...
/**
	...
	*/
	 ...
	 * @since 1.0.0
	 */
	public function plgattemptreportGetData($filters, $colNames, $rows_to_fetch, $limit_start, $sortCol, $sortOrder)
	{
		$db        = JFactory::getDBO();
		...
		{
			$sortCol   = $mainframe->getUserState("com_tjlms.attemptreport_table_sortCol", '');
			$sortOrder = $mainframe->getUserState("com_tjlms.attemptreport_table_sortOrder", '');
		}
		...
		foreach ($items as $ind => $attempt)
		{
			...
			if (in_array('last_accessed_on', $colToshow))
			{
				$techjoomlacommon = new TechjoomlaCommon;

				if ($attempt->last_accessed_on == '0000-00-00 00:00:00')
				{
					$attempt->last_accessed_on = '-';
				}

				if ($attempt->last_accessed_on != '-')
				{
					$attempt->last_accessed_on = $techjoomlacommon->getDateInLocal($attempt->last_accessed_on, 0, $date_format_show);
				}
			}
		}
	 ...
	 * @since    1.0
	 */
	public function plgattemptreportRenderPluginHTML($filters, $colNames, $rowsToFetch, $limit_start, $sortCol, $sortOrder, $action, $layout = 'default')
	{
		// Get data
		$resultData = $this->plgattemptreportGetData($filters, $colNames, $rowsToFetch, $limit_start, $sortCol, $sortOrder);
		...
		{
			$this->lessonFilter = $TjlmsModelReports->getLessonFilter();
		}
	...
	{
		$ColArray = array(
			'PLG_TJLMSREPORTS_ATTEMPTREPORT_ATTEMPT' => 'attempt',
			'PLG_TJLMSREPORTS_ATTEMPTREPORT_NAME' => 'name',
			'PLG_TJLMSREPORTS_ATTEMPTREPORT_USERNAME' => 'username',
			'PLG_TJLMSREPORTS_ATTEMPTREPORT_TIME_SPENT' => 'time_spent',
			'PLG_TJLMSREPORTS_ATTEMPTREPORT_LESSONIDEALTIME' => 'lessonIdealTime',
			'PLG_TJLMSREPORTS_ATTEMPTREPORT_LESSON_STATUS' => 'lesson_status',
			'PLG_TJLMSREPORTS_ATTEMPTREPORT_SCORE' => 'score',
			'PLG_TJLMSREPORTS_ATTEMPTREPORT_LAST_ACCESSED_ON' => 'last_accessed_on'
		);
