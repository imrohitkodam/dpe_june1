							...
							foreach ($colToshow as $eachColumn)
							{
								...
								if ($sortCol == $eachColumn && !empty($sortCol) && !empty($sortOrder) )
								{
									...
									<th class="th_<?php echo $eachColumn; ?> left <?php echo $class; ?>">
										...
										<?php $calHeading = 'PLG_TJLMSREPORTS_STUDENTCOURSEREPORT_' . $calHeading; ?>

										<a href="#" onclick="sortColumns(this); return false;" class="hasTooltip" title="<?php echo JText::_('PLG_TJLMSREPORTS_STUDENTCOURSEREPORT_SORT'); ?>" data-value="<?php echo $eachColumn; ?>"><?php echo JText::_($calHeading); ?><?php echo $icon; ?></a>
										...
										if (in_array($eachColumn,$showSearchToCol))
										{
											if ($eachColumn == 'title')
												...
												else if ($eachColumn == 'status')
												{
													if (isset($this->statusFilter))
													{
														echo JHtml::_('select.genericlist', $this->statusFilter, "search-filter-". $eachColumn , 'class="filter-input input-medium" size="1" onchange="getFilterdata();" name="filter_statusFilter"', "value", "text", isset($filters[$eachColumn]) ? $filters[$eachColumn] : '');
													}
												}
												else
												{
