...
class PlgTjlmsreportsLessonreport extends JPlugin
{
	/**
	...
	public function __construct(&$subject, $config)
	{
		...
		$extension    = 'plg_tjlmsreports_lessonreport';
		$base_dir     = JPATH_BASE;
		$language_tag = 'en-GB';
		$reload       = true;
...
/**
	...
	*/
	 ...
	 * @since 1.0.0
	 */
	public function plglessonreportGetData($filters, $colNames, $rows_to_fetch, $limit_start, $sortCol, $sortOrder)
	{
		...
		foreach ($colToshow as $eachCol)
		{
			switch ($eachCol)
			{
				...
				case 'timeSpentOnLesson':
					...
					break;
				case 'lessonIdealTime':
					$query->select('IF (l.ideal_time, SEC_TO_TIME(l.ideal_time * 60), "-") as lessonIdealTime');
					break;
				case 'lessonformat':
				case 'attempts_grade':
				case 'status':
		...
		foreach ($items as $ind => $lessonDetails)
		{
			...
			if (in_array('no_of_attempts', $colToshow))
			{
				...
				{
					$lessonDetails->no_of_attempts = JText::_('PLG_TJLMSREPORTS_LESSONREPORT_UNLIMITED');
				}
			}
			...
			if (in_array('status', $colToshow))
			{
				...
				if (property_exists($result, 'lesson_status'))
				{
			...
			if (in_array('score', $colToshow))
			{
				...
				if (property_exists($result, 'score'))
				{
			...
			if (in_array('attempts_grade', $colToshow))
			{
				...
				{
					case '0':
							$lessonDetails->attempts_grade = JText::_('COM_TJLMS_HIGHEST_ATTEMPT');
							break;
					case '1':
							$lessonDetails->attempts_grade = JText::_('COM_TJLMS_AVERAGE_ATTEMPT');
							break;
					case '2':
							$lessonDetails->attempts_grade = JText::_('COM_TJLMS_FIRST_ATTEMPT');
							break;
					case '3':
							$lessonDetails->attempts_grade = JText::_('COM_TJLMS_LAST_COMPLETED_ATTEMPT');
							break;
				}
			}
	 ...
	 * @since    1.0
	 */
	public function plglessonreportRenderPluginHTML($filters, $colNames, $rowsToFetch, $limit_start, $sortCol, $sortOrder, $action, $layout = 'default')
	{
		// Get data
		$resultData = $this->plglessonreportGetData($filters, $colNames, $rowsToFetch, $limit_start, $sortCol, $sortOrder);
	...
	{
		$ColArray = array(
			'PLG_TJLMSREPORTS_LESSONREPORT_ID' => 'id',
			'PLG_TJLMSREPORTS_LESSONREPORT_NAME' => 'name',
			'PLG_TJLMSREPORTS_LESSONREPORT_COURSETITLE' => 'courseTitle',
			'PLG_TJLMSREPORTS_LESSONREPORT_USERNAME' => 'username',
			'PLG_TJLMSREPORTS_LESSONREPORT_START_DATE' => 'start_date',
			'PLG_TJLMSREPORTS_LESSONREPORT_END_DATE' => 'end_date',
			'PLG_TJLMSREPORTS_LESSONREPORT_NO_OF_ATTEMPTS' => 'no_of_attempts',
			'PLG_TJLMSREPORTS_LESSONREPORT_ATTEMPTSDONE' => 'attemptsDone'
		);

		$ColArray['PLG_TJLMSREPORTS_LESSONREPORT_TIMESPENTONLESSON'] = 'timeSpentOnLesson';
		$ColArray['PLG_TJLMSREPORTS_LESSONREPORT_LESSONIDEALTIME'] = 'lessonIdealTime';
		$ColArray['PLG_TJLMSREPORTS_LESSONREPORT_ATTEMPTS_GRADE'] = 'attempts_grade';
		$ColArray['PLG_TJLMSREPORTS_LESSONREPORT_LESSONFORMAT'] = 'lessonformat';
		$ColArray['PLG_TJLMSREPORTS_LESSONREPORT_CONSIDER_MARKS'] = 'consider_marks';
		$ColArray['PLG_TJLMSREPORTS_LESSONREPORT_STATUS'] = 'status';
		$ColArray['PLG_TJLMSREPORTS_LESSONREPORT_SCORE'] = 'score';
