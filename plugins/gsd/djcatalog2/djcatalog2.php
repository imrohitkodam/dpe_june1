...
class plgGSDDJCatalog2 extends PluginBaseProduct
{
    ...
    protected function viewItem()
    {
        ...
        $currency = Djcatalog2HelperPrice::getCurrencyDefault();
    ...
    {
        $prices = Djcatalog2HelperPrice::getPrices($this->product->final_price, $this->product->price, $this->product->tax_rule_id, false, ComponentHelper::getParams('com_djcatalog2'));
        $productPrice = isset($prices['display']) ? $prices['display'] : 0;
        ...
        $query = $db->getQuery(true)
            ...
            ->where($db->quoteName('item_id') . '=' . $this->product->id);

        $db->setQuery($query);

        // If it's a variant, return it
        $variants = $db->loadObject();
        if ($variants->min && $variants->max)
        {
            $variants = array_values((array) $variants);

            // A variant price may be 0, we assume this is the original price, so we need to discard it and use the main product price
            $hasZeroPrice = array_filter($variants, function($v) {
                return (float) $v == 0;
            });
            if ($hasZeroPrice)
            {
                $variants[] = $productPrice;
                $variants = array_filter($variants, 'floatval');
            }
