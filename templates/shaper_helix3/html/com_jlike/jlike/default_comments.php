...
<form action="<?php echo htmlspecialchars($this->urldata->url, ENT_QUOTES, 'UTF-8');?>" method="post" id="jlike_comments" name="jlike_comments" enctype="multipart/form-data">
						...
						<div class="<?php echo $this->urldata->show_comments ? 'pull-left':'pull-right'; ?>">
							<div class="jlike_comment_sort">
								...
								<a data-toggle="dropdown" href="javascript:void(0)" class="jlike_comment_msg <?php echo $temp_class;?>">
									...
									<span>
										<?php  echo $this->urldata->show_comments ?  Text::_('COM_JLIKE_COMMENTS'): '<a href="'.htmlspecialchars($this->urldata->url, ENT_QUOTES, 'UTF-8').'" title="Click here to add comment"> '.Text::_('COM_JLIKE_COMMENTS').'</a>' ?>
									</span>
			...
			foreach($this->comments as $comment)
			{
				...
				<div id="jlcomment<?php echo $comment->annotation_id; ?>" class="media jlike_commentingArea jlike_renderedComment jlike_no_radius jlike_text_decoration" >
					...
					<?php if (!empty($comment->user_profile_url)) : ?>
						<a class="pull-left" target="_blank" href="<?php echo htmlspecialchars($comment->user_profile_url, ENT_QUOTES, 'UTF-8'); ?>" >
					<?php else:	?>
						<div class="pull-left">
					<?php endif; ?>
					...
					<div class="media-body jlike_media_body" >
						<span>
							<?php if (!empty($comment->user_profile_url)) : ?>
								<a href="<?php echo htmlspecialchars($comment->user_profile_url, ENT_QUOTES, 'UTF-8'); ?>">
							<?php endif; ?>
									<strong class="jlike-comment-author"><?php echo htmlspecialchars(ucwords($comment->name), ENT_QUOTES, 'UTF-8'); ?></strong>
							<?php if (!empty($comment->user_profile_url)) : ?>
								</a>
							<?php endif; ?>
						...
						<div class="jlike_comment_padding_top">
							...
							<!-- Comment added user & logged in user are the same then show edit comment  DPE HACK ALSO ADDED-->
							<?php if ($loged_user == $comment->user_id): ?>

								<div id="EditComment<?php echo $comment->annotation_id;?>" class="jlike_display_none" >
									<div id="wysiwyg"><div class="btns"><div class="category"><p  onclick="execCmd(this);"class="btn" data-cmd="undo"><i class="fas fa-undo"></i></p><p  onclick="execCmd(this);"class="btn" data-cmd="redo">				<i class="fas fa-redo"></i>	</p></div>	<div class="category"></div><div class="category">			<p class="btn" data-cmd="bold" onclick="execCmd(this);"><i class="fas fa-bold"></i></p><p class="btn" data-cmd="italic"  onclick="execCmd(this);"><i class="fas fa-italic"></i></p><p class="btn" data-cmd="underline" onclick="execCmd(this);"><i class="fas fa-underline" onclick="execCmd(this);"></i></p></div><div class="category"><p  onclick="execCmd(this);" class="btn" data-cmd="justifyLeft">				<i class="fas fa-align-left"></i></p><p  onclick="execCmd(this);"class="btn" data-cmd="justifyCenter"><i class="fas fa-align-center"></i></p><p  onclick="execCmd(this);" class="btn" data-cmd="justifyRight"><i class="fas fa-align-right"></i>		</p><p  onclick="execCmd(this);"class="btn" data-cmd="justifyFull"><i class="fas fa-align-justify"></i></p></div><div class="category"><p  onclick="execCmd(this);"class="btn" data-cmd="indent"><i class="fas fa-indent"></i></p><p  onclick="execCmd(this);" class="btn" data-cmd="outdent">				<i class="fas fa-outdent"></i></p></div><div class="category"><p  onclick="execCmd(this);"class="btn" data-cmd="insertUnorderedList"><i class="fas fa-list-ul"></i></p><p  onclick="execCmd(this);"class="btn" data-cmd="insertOrderedList"><i class="fas fa-list-ol"></i></p></div><div class="category"><p  onclick="execCmd(this);"  data-cmd="createlink" class="prompt btn"><i class="fas fa-link"></i></p><p  onclick="execCmd(this);"  data-cmd="insertimage" class="prompt btn"><i class="fas fa-image"></i></p></div></div></div>
