...
<script type="text/javascript">
			...
			jQuery.ajax({
				...
				success:function(data)
				{
					...
					for(var index=0;index<data.length;index++)
					{
						...
						html+='<strong>'+jlike.escapeHtml(titleCase(data[index]['name']))+'</strong><hr/>';
					}
					...
					for(index = 0; index < data.length; index++)
					{
						var html='<div id="jlcomment'+data[index]['annotation_id']+'" style="'+style+'"  class="media row-fluid  jlike_commentingArea  jlike_renderedComment">';
							...
							<?php else:	?>
								html+= '<span>';
							<?php endif;	?>
								html+= "<strong>"+jlike.escapeHtml(titleCase(data[index]['name']))+"</strong>";
							<?php if (!empty($comment->user_profile_url)) : ?>
								html+= '</a>';
							<?php else:	?>
								html+= '</span>';
		...
		*/
		function addCommentArea(likecontainerid, parent_id,margin_left,width,commentbtn_margin,threadlevel)
		{
			...
			//get user info
			//get user info
			profile_url=<?php echo json_encode($userInfo->user_profile_url); ?>;
			avtar=<?php echo json_encode($userInfo->avtar); ?>;
			user_name=<?php echo json_encode(ucfirst($userInfo->name)); ?>;
			...
			var html ='<div id="jlcomment'+textAreaId+'" style=" margin-left:'+margin_left+'%;" class="media jlike_add_comment jlike_commentingArea jlike_commentBox clone_othere_reply_box jlike_no_radius" >';
				...
				<?php else:	?>
					html+= '<div class="pull-left">';
				<?php endif;	?>
					html+= //'<img class="img-rounded " src="'+avtar+'" alt="Smiley face" width="36px" height="auto">
					'</a>';
				<?php if (!empty($userInfo->user_profile_url)) : ?>
					html+= '</a>';
				<?php else:	?>
				...
				<?php else:	?>
					html+= '<span>';
				<?php endif;	?>
					html+= "<strong>" + jlike.escapeHtml(user_name) + "</strong>";
				<?php if (!empty($comment->user_profile_url)) : ?>
					html+= '</a>';
				<?php else:	?>
					html+= '</span>';
				...
				// SMILEY BUTTON DPE HACK FOR COMMENT 
				html+=		'<div id="wysiwyg"><div class="btns"><div class="category"><p  onclick="execCmd(this);"class="btn" data-cmd="undo"><i class="fas fa-undo"></i></p><p  onclick="execCmd(this);"class="btn" data-cmd="redo">				<i class="fas fa-redo"></i>	</p></div>	<div class="category">			<p class="btn" data-cmd="bold" onclick="execCmd(this);"><i class="fas fa-bold"></i></p><p class="btn" data-cmd="italic"  onclick="execCmd(this);">				<i class="fas fa-italic"></i></p><p class="btn" data-cmd="underline" onclick="execCmd(this);"><i class="fas fa-underline" onclick="execCmd(this);"></i></p>		</div><div class="category"><p  onclick="execCmd(this);" class="btn" data-cmd="justifyLeft">				<i class="fas fa-align-left"></i></p><p  onclick="execCmd(this);"class="btn" data-cmd="justifyCenter"><i class="fas fa-align-center"></i></p><p  onclick="execCmd(this);" class="btn" data-cmd="justifyRight"><i class="fas fa-align-right"></i>		</p><p  onclick="execCmd(this);"class="btn" data-cmd="justifyFull"><i class="fas fa-align-justify"></i></p></div><div class="category"><p  onclick="execCmd(this);"class="btn" data-cmd="indent"><i class="fas fa-indent"></i></p><p  onclick="execCmd(this);" class="btn" data-cmd="outdent">				<i class="fas fa-outdent"></i></p></div><div class="category"><p  onclick="execCmd(this);"class="btn" data-cmd="insertUnorderedList"><i class="fas fa-list-ul"></i></p><p  onclick="execCmd(this);"class="btn" data-cmd="insertOrderedList"><i class="fas fa-list-ol"></i></p></div><div class="category"><p  onclick="execCmd(this);"  data-cmd="createlink" class="prompt btn"><i class="fas fa-link"></i></p><p  onclick="execCmd(this);"  data-cmd="insertimage" class="prompt btn"><i class="fas fa-image"></i></p></div></div><div class="jlike_textarea jlike-mention_'+likecontainerid+'" taggable tag_editor editor_0 jl_padding" tag_editor_for="0" id="CommentText'+textAreaId+'" contenteditable="true" required="required" onkeyup="characterLimit(id, <?php echo $maxlength; ?>)" ></div></div>';
				...
				html+=	'<button  type="button" class="btn btn-success pull-right" onclick="SaveNewComment(this,\''+likecontainerid+'\')"><?php echo Text::_('COM_JLIKE_COMMENT') ?></button>';
				html+=	"<button  type='button' class='btn pull-right jlike_cancel_btn' onclick='CancelNewComment(this)'><?php echo Text::_('COM_JLIKE_CACEL') ?></button> ";
				html+= '</div>';
				//COMMENT & CANCEL BUTTON
...
function execCmd(cmd, value) { 
	...
	if (cmd.hasClass("prompt")) {
			value = prompt('Enter the link here: ', 'http:\/\/');
		}

	document.execCommand(cmd.getAttribute('data-cmd'), false, value);
}

jQuery('.jlike_position_relative').click(function(){
