...
// DPE Hack
	...
	if (strpos($currentUrl, 'dfe-digital-standards') !== false) {
	...
	// Check user have permission to view records of assigned cluster
		if (!$user->authorise('core.manageall', 'com_cluster'))
		{
			...
			$userRoleId = $userModel->getRoleByUser($user->id, 'com_multiagency', 0);

			  $params = ComponentHelper::getParams('com_dpe');
			  $key = $params->get('multyagency_admin_role_id');

			if (!in_array($key, $userRoleId))
			{
...
<script>
    ...
    document.querySelectorAll("a.backUrl").forEach(link => {
        const currentUrl = encodeURIComponent(window.location.href);
        const separator = link.href.includes("?") ? "&" : "?";
        link.href += `${separator}urlBack=${currentUrl}`;
    });
});
