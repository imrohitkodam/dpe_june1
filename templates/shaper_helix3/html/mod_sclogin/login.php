...
if ($params->get('displayType') == 'modal')
{
    ...
    echo '<div class="sourcecoast sclogin-modal-links sclogin"><a ' . $loginClass . ' href="#login-modal" role="button" data-toggle="' . $modalName . '">' . Text::_('MOD_SCLOGIN_LOGIN') . '</a>';
    if ($showRegisterLinkInModal)
        echo $spacer . '<a ' . $registerClass . ' href="' . $helper->registerLink . '">' . Text::_('MOD_SCLOGIN_REGISTER_FOR_THIS_SITE') . '</a>';
    echo '</div>';
    ...
    <div class="sclogin sourcecoast" id="sclogin-<?php echo $module->id; ?>">
        ...
        <div class="row-fluid">
            ...
            if($params->get('socialButtonsOrientation') == 'top')
            {
                ...
                require(ModuleHelper::getLayoutPath("mod_sclogin", "joomlaLogin_" . $layout));
            }
            else
            {
                require(ModuleHelper::getLayoutPath("mod_sclogin", "joomlaLogin_" . $layout));
