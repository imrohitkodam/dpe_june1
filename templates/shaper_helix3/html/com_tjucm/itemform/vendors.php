...
if (!empty($tmpl))
{
	...
	$doc->addStyleSheet('media/com_tjucm/css/tjucm.css');
	$doc->addScript('templates/shaper_helix3/js/bootstrap.min.js');
	$doc->addStyleSheet('templates/shaper_helix3/css/custom.css');
	$doc->addStyleSheet('templates/shaper_helix3/css/bootstrap.min.css');
	$doc->addScript('templates/shaper_helix3/js/jquery.sticky.js');
	$doc->addScript('templates/shaper_helix3/js/main.js');
	$doc->addScript('templates/shaper_helix3/js/frontend-edit.js');
	$doc->addScript('media/system/js/frontediting.js');
}
...
foreach($userClusters as $key => $usecluster)
{
	$clusterData[$key] =  $usecluster->cluster_id;
}

if (($this->item->created_by != $user->id) && ($orgAdmin && !in_array($this->item->cluster_id, $clusterData) && ($this->item->created_by) ) )
{	
	...
	jQuery(function() {
		...
		jQuery(document).ready(function ()
		{
			...
			<?php if (!empty($tmpl)): ?>
				...
				<h3 class="rop-popup-header ml-20 mr-20 pb-2 fw-bold"><?php echo Text::_('COM_TJUCM_CORE_DATA_ADD_NEW_RECORD_TITLE'); ?></h3>
			...
			// Code to display the form
						echo $this->loadTemplate('extrafieldsvendors');
		...
		$enableUserOnbaord = $dpeParams->get('enableUserOnboard');
 
		if ($enableUserOnbaord && $this->client == 'com_tjucm.role')
		{
			echo $this->loadTemplate('extrauseronboarding');
		}
		?>
		<script>
...
jQuery("#jform_"+jQuery("#clusterFieldUniqueName").val()).change(function()
{
	...
	jQuery("#cluster_id").val(this.value);
		...
		jQuery(".tab-content .tab-pane").css("height", totalheight_form1-40);
	...
	function formHeightafterscroll(action_btn_form){
		...
		jQuery(".tab-content .tab-pane").css("height", totalheight_form-40);
