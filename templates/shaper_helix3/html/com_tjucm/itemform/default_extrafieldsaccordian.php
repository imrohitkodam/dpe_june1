	...
	foreach ($fieldSets as $fieldset)
	{
		...
		<div class="form-horizontal clear-both pull-left pb-10 w-100 dp-rop-form row">
			...
			foreach ($this->form_extra->getFieldset($fieldset->name) as $field)
			{
				...
				if(!empty($field->getAttribute('tags')))
				{
					...
					$tagnames = $temp->getTagNames(array($field->getAttribute('tags')));
						...
						if(!empty($tagnames[0]) && array_key_exists($fieldsArray, $tagnames[0]))
						{
							$fieldsArray[$tagnames[0]][] = $field;
						}
						else
						{
							$fieldsArray[$tagnames[0]][] = $field;
			...
			if(!empty($fieldsArray))
			{
				...
				<div class="w-100">
				<?php
					foreach ($fieldsArray as $key => $fieldTagarray)
					{
						$i =0;
						...
						foreach($fieldTagarray as $fieldTag)
						{
							...
							{
								$customColClass = 'col-12 ucmsubform';
								if($fieldTag->max == 1)
								{
								?>
...
<?php
			...
			<?php
				...
				foreach ($extraFieldArray as $field)
				{
				...
				<?php }
							...
							if (!$field->hidden)
							{
									$className = ($field->type == 'Spacer') ? 'w-100' : '';
									?>
								<div class="<?php echo $customColClass . ' ' . $className;  ?> custom-form-style-extra">
										<div class="form-group">
												<div class="col-sm-12 control-label w-100 text-left">
