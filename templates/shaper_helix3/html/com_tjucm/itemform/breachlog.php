				...
				<div class="row">
					<div class="row">
						<div class="col-sm-9  text-end">
							...
							<?php if(((!TjucmAccess::canView($ucmTypeId, $this->id, $user->id)) && (!$user->authorise('core.manageall', 'com_cluster')) && !$orgAdmin) && ($this->client == 'com_tjucm.breachlog' || $this->client == 'com_tjucm.dpialite')) {?>
								
									<input type="button" class="btn btn-warning mobile-space" value="<?php echo Text::_("COM_TJUCM_CANCEL_BUTTON"); ?>" onclick="toDashboard();" />

							<?php } 
							
							elseif ((!$user->authorise('core.manageall', 'com_cluster')) && !$orgAdmin && 
							($this->client == 'com_tjucm.dpialite') && !$this->item->id)
							{?>
...
<?php if($user->authorise('core.manageall', 'com_cluster') && in_array($this->client, ['com_tjucm.breachlog', 'com_tjucm.FOIlog', 'com_tjucm.sarlog'])) {?>
