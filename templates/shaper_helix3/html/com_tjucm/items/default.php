...
$clusterID = $jinput->input->get('cluster') ?? ''; // Shorter ternary syntax
...
</style><div id="ropCopyLoader" class="centerloader hide"></div>
...
<!--
	...
	//~ }
		...
		$backUrl = urldecode($input->get('urlBack', '', 'STRING'));

		if ($backUrl)
			{?>
				<div>
					<a class="fs-16 font-600 cursor-pointer" href="<?php echo htmlspecialchars($backUrl, ENT_QUOTES, 'UTF-8');?>"><i class="fa fa-arrow-left mr-10" aria-hidden="true"></i><?php echo Text::_('COM_DPE_BACK_BUTTON');?></a>
				</div><br>
			<?php }
									...
									<thead>
										<tr>
											...
											<?php }else{
												?>
												<th width="1%" class="">
													<input type="checkbox" name="checkall-toggle" value="" title="<?php echo Text::_('JGLOBAL_CHECK_ALL'); ?>" disabled />
												</th>
											<?php } ?>
											<?php
...
<!--
		...
		class="btn btn-success btn-small">
			...
			<!-- DPE Hack end -->
			<input type="hidden" id="urlBack" name="urlBack" value="<?php echo htmlspecialchars($backUrl, ENT_QUOTES, 'UTF-8'); ?>">
