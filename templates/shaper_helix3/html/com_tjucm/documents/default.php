...
<form action="<?php echo Uri::getInstance()->toString(); ?>" method="post"
	name="adminForm" id="adminForm">
		<div class="dp-search-filter">
			<div id="filter-progress-bar" class="dp-search-filter d-inline-block  float-left">
					<div class="btn-group pull-left mb-2 me-2">
							<div class="pull-left">
								<input type="text" class="d-inline-block" name="filter[search]" id="filter_search"
							...
							<div class="pull-left btn-group">
								<button class="btn btn-primary" type="submit" title="<?php echo Text::_('JSEARCH_FILTER_SUBMIT'); ?>"><span class="icon-search"></span></button>
								<button class="btn btn-default btn-basic qtc-hasTooltip rounded-0 ms-0" style="background-color: #efefef;" id="clear-search-button" onclick="document.getElementById('filter_search').value='';this.form.submit();" type="button" title="<?php echo Text::_('JSEARCH_FILTER_CLEAR'); ?>">Clear</button>
							</div>
					</div>
			</div>
			<?php 
				...
				if ( in_array($orgAdmin,$user->groups))
				{
					$genericSchoolId = ComponentHelper::getParams('com_dpe')->get('cluster_id');
					
					Table::addIncludePath(JPATH_ROOT . '/administrator/components/com_cluster/tables');
  					$clusterTable = Table::getInstance('Clusters', 'ClusterTable');
  					$clusterTable->load(array('id'=>$genericSchoolId));

					$generciSchoolData = (object) array( 'value'=> $clusterTable->id,'text'=>$clusterTable->name);
					$this->clusterList[] = $generciSchoolData;
				}

			?>
			<div class="btn-group float-left md-w-300px text-start d-inline-block">
				<?php
				echo HTMLHelper::_('select.genericlist', $this->clusterList, "filter[cluster_id]", 'class="input-medium" size="1" onchange="this.form.submit();"', "value", "text", $this->state->get('filter.cluster_id', $this->clusterList['1']->value));
				?>
			</div>
				...
				<table class="table table-striped" id="activityList">
					...
					<tbody>
						...
						<tr class="row<?php echo $i % 2; ?>">
							...
							$link = 'index.php?option=com_tjucm&view=document&id=' . $item->id . '&tmpl=component&cluster_id=' . $clusterId.'&generate_doc='.$orgAdmin;
							?>
								<a class="d-inline-block mr-4" href="javascript:void(0);" onclick="openDocumentPopup('<?php echo Route::_($link, false);?>')" title="<?php echo TEXT::_('COM_TJUCM_DOCUMENT_PREVIEW');?>" >
