<?php
   ...
   $staff           		    =in_array((int) $params->get('member_role_id', '0', 'INT'), $user->groups);
       ...
       if(($itemData->client == 'com_tjucm.dpialite' ) && (!$orgAdmin) &&( !$user->authorise('core.manageall', 'com_cluster'))) {
   	...
   	if (($fieldGroupTable->name == 'Main') && (count($user->groups) === 1))
   	{
         ...
         <div class="col-12 accordDetail">
            <div class="row">
               ...
               <?php }
                  ...
                  {
                  	?>
               <div class="col-12 freetext_content">
                  <?php 
                     }else
                     {?>
                           ...
                           <div class="form-group">
                              <?php 
                                 if ($fieldTags->type == 'Freetext')
                                 	{?>
                              <div class="field-label freetextMod"><?php echo html_entity_decode($fieldTags->getAttribute('freetext')); ?></div>
                              ...
                              <div class="field-data">
                                 <?php
                                    ...
                                    if($fieldTags->type == 'Dpechecklist'){
                                    		...
                                    		$fieldTags->value = ($fieldTags->value == 'todo') ? 'To-Do' : 
                                    		(($fieldTags->value == 'inprogress') ? 'In Progress' : 
                                    			(($fieldTags->value == 'done') ? 'Done' : $fieldTags->value));
                                    	}
                                    $output = $layout->render(array('fieldXml' => $xmlField, 'field' => $fieldTags));
                                 ...
                                 <div class="tj-wordwrap">
                                    <?php 
                                       if (isset($ticketConditionData->linkField) && $fieldTags->value && 'jform_'.$fieldTableLink->name == $fieldTags->id)
                                       {?>
                     ...
                     <div class="form-horizontal">
                        <div class="form-group">
                           <?php 
                              if ($field->type == 'Freetext')
                              	{?>
                           <div class="field-label freetextMod"><?php echo html_entity_decode($field->getAttribute('freetext')); ?></div>
                           ...
                           <div class="field-data">
                              <?php
                                 ...
                                 if($field->type == 'Dpechecklist'){
                                 			...
                                 			$field->value = ($field->value == 'todo') ? 'To-Do' : 
                                 			(($field->value == 'inprogress') ? 'In Progress' : 
                                 				(($field->value == 'done') ? 'Done' : $field->value));
                                 		}
                                 $output = $layout->render(array('fieldXml' => $xmlField, 'field' => $field));
...
<script type="text/javascript">
   ...
   if(hidetabs)
   {
   	jQuery('.detail-view-tabs li').each(function() {
