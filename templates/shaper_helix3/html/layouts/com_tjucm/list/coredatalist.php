					...
					<?php
						if ($canView || ($item->created_by == $user->id))
						{
							...
							$paramsDpe              = ComponentHelper::getParams('com_dpe');// Also known as field for vendor
 							$alsoKnownAsFieldId  = json_decode($params->get('alsoknownas'));


							if ($alsoKnownAsFieldId)
							{
								if (is_object($alsoKnownAsFieldId))
								{
									 $alsoKnownAsFieldId	 = $alsoKnownAsFieldId->com_tjucm_ropvendors_organisationname;
								}
								Table::addIncludePath(JPATH_ROOT . '/administrator/components/com_tjfields/tables');
							 	$fieldValueTable = Table::getInstance('fieldsvalue', 'TjfieldsTable', array('dbo', $db));
							 	$fieldValueTable->load(array('content_id' => $item->id,'field_id'=>$alsoKnownAsFieldId));
				
								if ($fieldValueTable->value)
								{
									$alsoKnwonAs = ' ('.$fieldValueTable->value.')';
								}
								else
								{
									$alsoKnwonAs ='';
									...
									<li class="list-group-item" data-ucm-id="<?php echo $ucmDataId; ?>" data-org-name="<?php echo $orgName; ?>">
										<?php echo $output; ?><?php echo $alsoKnwonAs.' ('.$orgName.') ' ?>
									</li>
									<?php
								}
								else
