...
<div class="field-calendar">
    ...
    // DPE hack
    if ($value == '24-07-2018 22:59'){
        $currentTimestamp = time();

        $formattedDateTime = date("d-m-Y H:i", $currentTimestamp);
...
<script type="text/javascript">
    // IN JOOMLA4 Chosen is not workable for calender so  chossen is removed.
    jQuery(document).ready(function() {

    setTimeout(function(){
    var inputElement = jQuery('#<?php echo $id; ?>').val();

       

        jQuery(".time .chosen-container.chosen-container-single").remove();
        jQuery(".time-hours").css('display','block');   
        jQuery(".time-minutes").css('display','block');   
        jQuery(".ampm-select").css('display','block');   
},500);
