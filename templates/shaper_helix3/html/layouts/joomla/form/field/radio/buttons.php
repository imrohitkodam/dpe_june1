...
$blockStart  = $isBtnGroup ? '' : '<div class="form-check d-inline-block ps-0">';
$blockEnd    = $isBtnGroup ? '' : '</div>';

// Add the attributes of the fieldset in an array
$containerClass = trim($class . ' radio mb-0' . ($readonly || $disabled ? ' disabled' : '') . ($readonly ? ' readonly' : ''));
...
if ($required) {
    $attribs[] = 'class="required radio mb-0"';
}
...
<fieldset <?php echo implode(' ', $attribs); ?>>
    ...
    <div class="<?php echo $containerClass; if(str_contains(trim($class), 'tj_btn_group') || str_contains($containerClass, 'radio')){echo ' create-group';}?>">
        ...
        <?php foreach ($options as $i => $option) : ?>
            <?php echo $blockStart; ?>
                ...
                <label for="<?php echo $oid; ?>" class="<?php echo trim($optionClass); ?> btn btn-default"<?php echo $style; ?>>
