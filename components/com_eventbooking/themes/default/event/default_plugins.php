...
?>
	<div id="eb-plugins-output" class="clearfix">
		<?php
		echo HTMLHelper::_('bootstrap.startTabSet', 'eb-event-plugins-output-' . $this->item->id, ['active' => 'eb-plugin-page-' . $this->item->id . '-0', 'recall' => true]);
		...
		foreach ($this->plugins as $plugin)
		{
			...
			{
				echo HTMLHelper::_('bootstrap.addTab', 'eb-event-plugins-output-'. $this->item->id, 'eb-plugin-page-' . $this->item->id . '-' . $count, $plugin['title']);
				echo $plugin['form'];
