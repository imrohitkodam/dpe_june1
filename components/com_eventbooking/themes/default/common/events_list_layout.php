...
<div class="<?php echo ($type == '0')?'eventpage-container':'eventpage-container1'?>">
		...
		<?php if($event->individual_price != '0.00'){?>
			...
			<div class="event-price-group">
				...
				<?php if ($event->early_bird_discount_type){
					if($event->early_bird_discount_date  != '0000-00-00 00:00:00')
					{
	...
	<div class="event-actions1">
		...
		<?php if($event->price_text == 'Free'){?>
			<!-- DPE Hack -->
			<a href="<?php echo ($event->registration_handle_url != '') ? $event->registration_handle_url : Route::_('index.php?option=com_eventbooking&task=register.individual_registration&event_id=' . $event->id . '&Itemid=' . $this->Itemid, false); ?>"
