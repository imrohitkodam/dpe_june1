	...
	function submitbutton(pressbutton) {
		if (pressbutton == 'save') {
			...
			if (regex_validate.test(sanitized) == false) {
				alert('".Text::_("COM_API_INVALID_DOMAIN_MSG")."');
				return false;
			}
		}
...
<h1 class="componentheading"><?php echo Text::_('COM_API_COMPONENT_HEADING');?></h1>
<h2 class="contentheading"><?php echo $this->key->id ? Text::_('COM_API_EDIT_KEY_PAGE_TITLE') : Text::_('COM_API_NEW_KEY_PAGE_TITLE');?></h2>
<form action="index.php" method="post" name="adminForm" class="api_key_form">
	<p>
		<label class="api_form_label" for="domain"><?php echo Text::_('COM_API_DOMAIN');?>:</label>
		<input type="text" class="inputbox api_form_input" name="domain" size="55" value="<?php echo $this->key->domain;?>" />
		<?php echo JHTML::tooltip(Text::_('COM_API_DOMAIN_TOOLTIP'), Text::_('COM_API_DOMAIN')); ?>
	</p>
	<?php if ($this->key->hash) : ?>
		<p>
			<label class="api_form_label"><?php echo Text::_('COM_API_KEY');?>:</label>
