...
class ComKoowaDispatcherAuthenticatorJwt extends KDispatcherAuthenticatorJwt
{
    ...
    protected function _initialize(KObjectConfig $config)
    {
        $config->append(array(
            ...
            'max_age' => Factory::getConfig()->get('lifetime') * 60,
            'options' => array(
                'action'       => Factory::getApplication()->isClient('site') ? 'core.login.site' : 'core.login.admin',
                'autoregister' => false,
                'type'         => 'jwt'
            ),
        ));
    ...
    protected function _loginUser($username, $data = array())
    {
        ...
        $parameter        = Factory::getApplication()->isClient('administrator') ? 'admin_language' : 'language';
        $data['language'] = $this->getUser($username)->get($parameter);

        $options = $this->_options;

        JPluginHelper::importPlugin('user');
        $results = Factory::getApplication()->triggerEvent('onUserLogin', array($data, $options));
        ...
        if (in_array(false, $results, true) == false)
        {
            ...
            $this->getObject('event.publisher')
                 ->publishEvent('onAfterUserLogin', array('user' => Factory::getUser($username)), Factory::getApplication());
