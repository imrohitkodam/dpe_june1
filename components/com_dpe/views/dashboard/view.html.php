 ...
 * @since  1.6
 */
class DpeViewDashboard extends HtmlView
{
	protected $state;
	...
	public function display($tpl = null)
	{
		...
		if($input->get('cluster_id', '', 'INT'))
		{
			$clusterId = $input->get('cluster_id', '', 'INT');
		...
		if ($subUserExist && !$this->user->authorise('core.manageall', 'com_cluster'))
		{
			...
			if (!RBACL::check($this->user->id, 'com_cluster', 'core.viewChecklist', 'com_multiagency', $this->clusterId))
			{
				$app->enqueueMessage(Text::_('JERROR_ALERTNOAUTHOR'), 'error');
