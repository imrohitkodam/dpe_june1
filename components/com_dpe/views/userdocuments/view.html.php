...
/**
 * DPE user documents view class.
 *
 * @since  __DEPLOY_VERSION__
 */
class DpeViewUserDocuments extends BaseHtmlView
{
	/**
	 * The user object
	 *
	...
	public function display($tpl = null)
	{
		...
		if (!$currentUserSuperUser && !$this->user->authorise('core.manageall', 'com_cluster'))
		{
			// Check user has permission for document added cluster
			$this->doucmentId         = $app->input->get('document_id', '', 'INT');

			// Get lesson id by providing content id
			...
			$jlikeContentTable->load(array('id' => $this->doucmentId, 'element' => 'com_tjlms.lesson'));
			...
			$manageLessons = RBACL::check($this->user->id, 'com_cluster', 'core.manage.lessons', 'com_tjlms', $table->cluster_id);
