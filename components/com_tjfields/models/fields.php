...
class TjfieldsModelFields extends ListModel
{
	...
	//  Dpe Hack to get field value by field ID

	/**
	 * Return feedback value of the fields  .
	 *
	 * @param    Integer $fieldId is used to get feedback value .
	 * 
	 ...
	 */
		public function getFieldValueByFieldId($fieldId)
		{
			...
			$query->select('id,feedback,options,value,ordering');
	...
	/**
	 * Return feedback value of the fields by field value  .
	 *
	 * @param    Integer $fieldId is Field id .
	 * 
	 * @param    String $fieldValue is field value .
	 * 
	 * @return    array of data 
	 */
		public function getFieldValueByFieldValue($fieldId, $fieldValue)
		{ 
			if (!$fieldId)
			{
				return false;
			...
			$query->from('`#__tjfields_options`');
			$query->where($db->quoteName('field_id') . '= ' . (int) $fieldId);
			$query->where($db->quoteName('value') . '= "' . $fieldValue.'"');
			$db->setQuery($query);
