...
class TjfieldsControllerFields extends FormController
{
	...
	/**
	 * Method to get fields list as per UCM type
	 *
	 * @return   null
	 *
	 * @since    1.6
	 */
	public function getUcmFieldsUsingType()
	{
		$jinput    = Factory::getApplication()->input;
		$ucmTypeId = $jinput->post->get('ucm_id', '', 'int');
		...
		$query->from($db->quoteName('#__tjfields_fields', 'tf'));
		$query->where($db->quoteName('tf.state') . '= 1');

		// Subquery to get the client from #__tj_ucm_types
		$subQuery = $db->getQuery(true);
		$subQuery->select('tut.unique_identifier');
		$subQuery->from($db->quoteName('#__tj_ucm_types', 'tut'));
		$subQuery->where($db->quoteName('tut.id') . '= ' . (int) $ucmTypeId);

		$query->where($db->quoteName('tf.client') . ' = (' . $subQuery . ')');
