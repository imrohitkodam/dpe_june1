...
class TjfieldsHelper
{
	...
	public function FetchDatavalue($data)
	{
		...
		foreach ($field_data_value as $k => $data)
		{
		    ...
		    elseif ($data->type == "multi_select" || (($data->type == "related" || $data->type == "tjlist" || $data->type == "assignee") && $multipleValueField))
		    {                   
		        $fieldDataValue[$data->field_id]->value[] = $data->value;
		        $fieldDataValue[$data->field_id]->field_id = $data->field_id;
		    }
		...
		foreach ($fieldDataValue as $fdata)
		{
			if (!empty($fdata))
			{
				...
				elseif ($fdata->type == "multi_select" || (($fdata->type == "related" || $fdata->type == "tjlist" || $fdata->type == "assignee") && $multipleValueField))
				{
					$values = array();
	...
	public function getFieldData($fname = '', $fid = '')
	{
		...
		$query->select($db->quoteName(array('id', 'type', 'name', 'label', 'params','group_id')));
	...
	public function saveFieldsValue($data)
	{
		...
		foreach ($data['fieldsvalue'] as $fieldName => $fieldValue)
		{
			...
			{
				// DPE hack start
				// If don't have field access then check user is assignee
				...
				$assignedUsers = $dpeMainHelper->getFieldValues($user->id, $data['content_id'], $field->client);
				...
				$ucmTypeTable->load(array('unique_identifier' => $field->client));

				// Check the edit permission
				$canEdit = TjucmAccess::canEdit($ucmTypeTable->id, $data['content_id']);

				if (empty($assignedUsers) && $canEdit == null)
				{
					continue;
				}
				// DPE hack end
			...
			if ($field->type == 'tjfile' || $field->type == 'image' || $field->type == 'captureimage')
			{
			...
			elseif ($field->type == 'ucmsubform' && is_array($fieldValue))
			{
				...
				$ucmSubformClientTmp = explode('_', str_replace("com_tjucm_", '', array_key_first((array)$fieldValue[array_key_first($fieldValue)])));
	...
	private function saveMediaFieldData($fieldValue, $client, $contentId, $fieldId, $fieldStoredValues = array())
	{
		...
		if (($fieldTable->type == 'tjfile' || $fieldTable->type == 'image') && empty($fieldValue['name']) && empty($fieldValue['tmp_name']) && empty($fieldValue['size']))
		{
			return false;
		}

		if (($fieldTable->type == 'captureimage') && is_array($fieldValue) && empty($fieldValue['name']) && empty($fieldValue['tmp_name']) && empty($fieldValue['size']))
		{
			return false;
		}
		elseif ($fieldTable->type == 'captureimage')
		{
	...
	public function getMediaUrl($fileName, $extraUrlParamsArray = array(), $renderer = 'download')
	{
		if (!empty($fileName))
		{
			if ($renderer == 'download')
			{
				...
				// DPE hack for tjfile type path is converted to nonsef
				$encodedFileName = base64_encode($fileName);
				$basePathLink = 'index.php?option=com_tjfields&task=getMediaFile&fpht=';
				$mediaURL = Uri::base() . substr($basePathLink . $encodedFileName . $extraUrlParams, strlen(Uri::base(true)));
	 ...
	 */
	public function getRadioOptions($field_id)
	{
		if(!$field_id)
		{
			return false;
