...
jQuery.extend(tjrContentUI.report, {
		...
		promise.fail(
			...
			function(response) {
				...
				else
				{
					...
					// DPE hack  If add todo plug is enabled then try to add a column of checkboxes and can go in core
					if (
					  typeof tjutilityaddTodo != 'undefined' &&
					  jQuery('body').find('.td-addtodo').length > 0
					)
					{
						tjutilityaddTodo.addColumn('report-table');
						tjutilityaddTodo.btnAddTodo();
					}
					...
					if (tjrContentUI.report.searchToggle)
					{
						...
						jQuery('#topFilters').show();
								...
								{ 
									jQuery('#filters_allUser').css('display','none');
									jQuery('.fill_all_user').css('display','none');
								}
								//hack end
				}
			}
	...
	loadReport: function(selectedElem, extension)
	{
		...
		// Get the filterstags value
		var filterstags = jQuery('#filterstags').val();

		// Check if filterstags is an array or a comma-separated string
		if (typeof filterstags === 'string') {
		    // Convert string to array by splitting on commas
		    filterstags = filterstags.split(',').map(function(item) {
		        return item.trim(); // Ensure no extra spaces around values
		    });
		}

		// Build the filters[tags] query string
		var filtersTagsParams = '';
		if (Array.isArray(filterstags)) {
		    filterstags.forEach(function(value, index) {
		        filtersTagsParams += '&filters[tags][' + index + ']=' + encodeURIComponent(value);
		    });
		}

		// Append the filtersTagsParams to the action URL
		var action = jQuery(document.adminForm).attr('action');
		var newAction = action + '&reportId=' + reportId + filtersTagsParams;
	...
	{
		jQuery("#filterstags").val([]).trigger("chosen:updated");

	},removecluster : function(){
		const selectElement = jQuery('select').filter(function() {
		    return jQuery(this).find('option').filter(function() {
		        return jQuery(this).text().trim() === 'Select Organisation';
		    }).length > 0;
		});

		if (selectElement.length) {
			
		    jQuery("#" + selectElement.attr('id')).val(jQuery('#' + selectElement.attr('id') + ' option:first').val());
		    jQuery("#" + selectElement.attr('id')).trigger('chosen:updated');
		}
