...
class JFormFieldAgencies extends JFormFieldList
{
	...
	protected function getOptions()
	{
		...
		$query->from('`#__tj_clusters` AS c');
		$query->join('INNER', $db->quoteName('#__tjmultiagency_multiagency', 'ml') . ' ON (' . $db->quoteName('ml.id') . ' = ' . $db->qn('c.client_id') . ')');
		...
		$query->order($db->escape('c.name ASC'));
		$db->setQuery($query);
		...
		$options = array();
		$options[] = HTMLHelper::_('select.option', "", Text::_('COM_JLIKE_ORG_SELECT'));

		foreach ($agencies as $agency)
		{
			$options[] = HTMLHelper::_('select.option', $agency->id, $agency->name);
		}
