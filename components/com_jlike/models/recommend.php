...
class JlikeModelRecommend extends ListModel
{
	...
	public function deleteTodo($todo_id)
	{
		...
		// DPE Hack for  send recomendation notification
/**
	 * Function to send a Recommendation Notification
	 *
	 * @param   string   $socialIntegration  integration
	 * @param   string   $body               notification_msg
	 * @param   integer  $assignedTo         owner_id
	 * @param   Array    $data               Array of data
	 * @param   Array    $elementData        Array of elementData
	 * @param   Array    $options            Array of plug name and plug type
	 *
	 * @return  void
	 * 
	 * @since  __DEPLOY_VERSION__
	 */
	public function sendRecommendationNotification($socialIntegration, $body, $assignedTo, $data = array(), $elementData = array(), $options = array())
	{
		$comJlikeSubHelper = new ComjlikeSubHelper;

		// Get email content from respective plugin
		PluginHelper::importPlugin($options['plg_type'], $options['plg_name']);
		$notificationDetails = Factory::getApplication()->triggerEvent('ongetjlike_dpeRecommendationNotificationDetails',
			array($options, $data, $elementData)
		);
		...
		{   // DPE HACK
			$replacements                     = new stdClass;
			$replacements->notification       = new stdClass;
			$replacements->notification->user = Factory::getUser($assignedTo)->name;
			$replacements->notification->msg  = $body;
		...
		$result = $comJlikeSubHelper->sendNotificationToUsers($socialIntegration, $body, $assignedTo, $options);
		
		return $result;
	}

	/**
	 * Function to send a Todo completion Notification
	 *
	 * @param   string   $socialIntegration  integration
	 * @param   string   $body               notification_msg
	 * @param   integer  $receiver           todo assigner 
	 * @param   Array    $todoData           Array of todo data
	 * @param   Array    $options            Array of plug name and plug type
	 *
	 * @return  void
	 * 
	 * @since  __DEPLOY_VERSION__
	 */
	public function sendTodoCompleteNotification($socialIntegration, $body, $receiver, $todoData = array(), $options = array())
	{
		$comJlikeSubHelper = new ComjlikeSubHelper;

		if ($todoData['cc_users'])
		{
			$todoData['cc_users'] = explode(',', $todoData['cc_users']);
			array_push($todoData['cc_users'], $receiver);
			$receiver = $todoData['cc_users'];
		}

		// Get email content from respective plugin
		PluginHelper::importPlugin($options['plg_type'], $options['plg_name']);
		$notificationDetails = Factory::getApplication()->triggerEvent('getjlike_dpeTodoCompleteNotificationDetails',
			array($options, $todoData)
		);
		...
		{  // DPE HACK
			$replacements                      = new stdClass;
			$replacements->notification        = new stdClass;
			$replacements->todo                = new stdClass;
			$replacements->subject             = new stdClass;
			$replacements->notification->owner = Factory::getUser($todoData['assigned_by'])->name;
			$replacements->notification->user  = Factory::getUser($todoData['assigned_to'])->name;
			$replacements->todo->title         = $todoData['title'];
			$replacements->subject->user       = Factory::getUser($todoData['assigned_to'])->name;

			$optionsRegistryObj = new Registry;
			$optionsRegistryObj->set('subject', $replacements->notification);

			$options['notifyClient']       = 'jlike';
			$options['notifyKey']          = 'jlike.todo.complete';
			$options['replacementsObj']    = $replacements;
			$options['optionsRegistryObj'] = $optionsRegistryObj;
		}

		$comJlikeSubHelper->sendNotificationToUsers($socialIntegration, $body, $receiver, $options);
	}
