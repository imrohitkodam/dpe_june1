...
class JlikeModelRecommendations extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		$query->select('c.element as client');
		...
		// Join over the created by field 'Assigned to'
		$query->join('LEFT', '#__users AS usa ON usa.id = a.assigned_to'); //DPE hack

		// Extend getlistQuery using plugin trigger
		Factory::getApplication()->triggerEvent('onRecommendationsModelGetListQuery', array($query, $this));
		...
		if ($this->getstate("done_date", ''))
		{
			$done_date = $this->getstate("done_date", '');
			$query->where('a.done_date = ' . $db->quote($done_date));
		}
		// DPE HACK
		...
		// DPE Hack: this condition '$status != "O"' added to get overdue result and overdue filter is not core filter
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.title LIKE ' . $search . ' OR users.username LIKE ' . $search . ' OR users.email LIKE ' . $search . ' OR usa.name LIKE ' . $search . ')'); //DPE hack
			}
		}
		// DPE Hack to get filter date
		...
		$records = $this->getState('filter.records');

		if (!empty($records))
		{
			if ($records === 'assigned_by')
			{
				$query->where('a.assigned_by = ' . $db->quote($userid));
			}
			elseif ($records === 'assigned_to')
			{
				$query->where('a.assigned_to = ' . $db->quote($userid));
			}
		}
		...
		else
		{
			...
			// DPE Hack to get filter assigned to and assigned by 

			if ($orderCol == 'a.assigned_to' )
			{
					$query->select('au.name');
					$query->join('LEFT', '#__users AS au ON au.id = a.assigned_to');
			}
			if ($orderCol == 'a.assigned_by')
			{
					$query->select('au.name');
					$query->join('LEFT', '#__users AS au ON au.id = a.assigned_by');
			}
		
			if ($orderCol == 'a.assigned_to' || $orderCol == 'a.assigned_by')
			{
				$orderCol = 'au.name';
			}
			// DPE Hack end
			...
			{
				// DPE Hack to show date desc date by default
	...
	/**
	 * Function to get the todo id of the first todo always
	 *
	 * @param   array  $elementId  element id
	 * 
	 * @param   array  $assignedTo  assigned user id
	 *
	 * @return  array;
	 *
	 * @since  __DEPLOY_VERSION__
	 */
	public function getTodoId($elementId, $assignedTo)
	{
		$db = Factory::getDbo();
		...
		$query->select('DISTINCT a.id')
		    ...
		    ->leftJoin($db->quoteName('#__jlike_content', 'c') . ' ON c.id = a.content_id')
		    ->leftJoin($db->quoteName('#__users', 'uc') . ' ON uc.id = a.checked_out')
		    ->leftJoin($db->quoteName('#__users', 'users') . ' ON users.id = a.created_by')
		    ->where('a.assigned_to = ' . $db->quote($assignedTo))
		    ->where('c.element_id = ' . $db->quote($elementId))
		    ->order('a.id ASC');
