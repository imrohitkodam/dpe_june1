...
class ComDocmanControllerBehaviorCompressible extends KControllerBehaviorAbstract
{
    ...
    public function isSupported()
    {
        ...
        if ($supported && \Joomla\CMS\Factory::getApplication()->isClient('site')) {
            $menu = \Joomla\CMS\Factory::getApplication()->getMenu()->getActive();

            if ($menu) {
                $supported = $supported && $menu->getParams()->get('allow_multi_download', false);
            }
        }
    ...
    /**
     * Get documents from archive string
     * 
     * @param string $archive Name of the generated archive
     * @return KModelEntityInterface
     */
    public function getDocuments($archive)
    {
        ...
        return $this->getModel()->id($ids)->fetch();
    }
    ...
    protected function _actionCompress(KControllerContext $context)
    {
        ...
        if ($query->has('Itemid') && $query->has('slug')) {
            $page = \Joomla\CMS\Factory::getApplication()->getMenu()->getItem($query->Itemid);
