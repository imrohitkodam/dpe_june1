...
class ComDocmanControllerToolbarList extends ComKoowaControllerToolbarActionbar
{
    public function getCommands()
    {
        if($this->getController()->canAdd()) {
            ...
            $page = \Joomla\CMS\Factory::getApplication()->getMenu()->getItem($this->getObject('request')->query->Itemid);

            if ($page && ($page->getParams()->get('allow_category_add', 1) || $this->getController()->canAdmin())) {
    ...
    protected function _afterBrowse(KControllerContextInterface $context)
    {
        ...
        $combined = $this->getObject('request')->query->combined;

        $show = $combined || !empty($slug) || !empty($filter);
        ...
        if ($layout !== 'default' && $show && $this->getObject('com://site/docman.controller.behavior.compressible')->isSupported()) {
            $this->addCommand('download', array(
                ...
                'href'    => 'view=download',
                'attribs' => array(
                    'class' => array('btn btn-secondary k-js-multi-download k-is-disabled')
                )
            ));
        }
    }
