...
class ComDocmanControllerSubmit extends ComKoowaControllerModel
{
    ...
    protected function _initialize(KObjectConfig $config)
    {
        $config->append(array(
            'behaviors' => array(
                ...
                'restrictable' => array('redirect_url' => JURI::base())
            ),
    ...
    protected function _setData(KControllerContextInterface $context)
    {
        ...
        $page       = \Joomla\CMS\Factory::getApplication()->getMenu()->getItem($this->getRequest()->query->Itemid);
        ...
        $data->enabled = $page->getParams()->get('auto_publish') ? 1 : 0;

        if (empty($data->storage_type)) {
            $data->storage_type = $data->storage_path_remote ? 'remote' : 'file';
        }

        $categories = (array) $page->getParams()->get('category_id');

        if (!$data->docman_category_id) {
            $data->docman_category_id = count($categories) ? $categories[0] : 0;
        }

        if (!in_array($data->docman_category_id, $categories) && $page->getParams()->get('category_children'))
        {
            $state = array(
