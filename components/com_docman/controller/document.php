...
class ComDocmanControllerDocument extends ComKoowaControllerModel
{
    ...
    protected function _initialize(KObjectConfig $config)
    {
        $config->append(array(
            ...
            'behaviors' => array(
                ...
                'restrictable' => array('actions' => array('edit', 'delete', 'add'), 'tables' => array('documents'), 'notify' => false)
            )
        ));
    ...
    /**
     * Checks to see if we need to take action based on layout in the query
     *
    ...
    protected function _beforeRender(KControllerContextInterface $context)
    {
        ...
        {
            // Joomla 4 pushes menu query information into query
            if ($this->isDispatched() && $request->query->has('layout')
                && in_array($request->query->get('layout', 'cmd'), ['gallery', 'table'])
            ) {
    ...
    protected function _checkDownloadLink(KControllerContextInterface $context)
    {
            ...
            && $query->view === 'document' && $this->getView()->getLayout() === 'default')
        {
            ...
            if ($menu->getParams()->get('document_title_link') === 'download'
