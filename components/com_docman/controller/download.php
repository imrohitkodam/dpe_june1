...
class ComDocmanControllerDownload extends ComKoowaControllerModel
{
    ...
    public function execute($action, KControllerContextInterface $context)
    {
        ...
        {
            if ($exception instanceof KHttpExceptionForbidden)
            {
                ...
                if ($request->isSafe() && $request->getFormat() == 'html' && $context->getName() == 'before.render')
                {
                    ...
                    $url = (string) $context->request->getReferrer();

                    if (!$url) {
                        $url = (string) $context->request->getSiteUrl();
                    }

                    $message = $this->getObject('translator')->translate('You are not authorized to download the selected file');

                    $this->getResponse()->setRedirect(JRoute::_($url, false), $message, 'error')->send();
                }
            }
    ...
    public function getRequest()
    {
        ...
        {
            //Set force download
            $params = \Joomla\CMS\Factory::getApplication()->getMenu()->getActive()->getParams();
