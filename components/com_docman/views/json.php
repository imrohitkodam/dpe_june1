...
class ComDocmanViewJson extends KViewJson
{
    ...
    protected function _getDocument(KModelEntityInterface $document)
    {
        ...
        if ($document->storage_type === 'file')
        {
            ...
            if ($document->storage && $width = $document->storage->width) {
                $data['file']['width'] = $width;
            }

            if ($document->storage && $height = $document->storage->height) {
        ...
        if ($this->isCollection())
        {
            ...
            if ($limit && $total-($limit + $offset) > 0)
            {
                $output['links']['next'] = array(
                    'href' => $this->_getPageUrl(array('limitstart' => $limit+$offset)),
                    'type' => 'application/json; version=1.0',
                );
            }
            ...
            if ($limit && $offset && $offset >= $limit)
            {
                $output['links']['previous'] = array(
                    'href' => $this->_getPageUrl(array('limitstart' => max($offset-$limit, 0))),
                    'type' => 'application/json; version=1.0',
                );
            }
        }
