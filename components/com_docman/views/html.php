...
class ComDocmanViewHtml extends ComKoowaViewHtml
{
    ...
    {
        return \Joomla\CMS\Factory::getApplication()->getPathway();
    }
    ...
    protected function _generatePathway($category = null, $document = null)
    {
        ...
        if (!in_array($query['view'], array('flat')) && $category && $query_slug !== $category->slug)
        {
            ...
            // Link category for document view, category filter for search view, no link if displaying a category
            ...
            $link = in_array($view, array('list', 'tree')) ? '' : JRoute::_('index.php?'.http_build_query($vars, '', '&'));
