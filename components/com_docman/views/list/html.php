...
class ComDocmanViewListHtml extends ComDocmanViewHtml
{
    ...
    public function setParentCategory(KModelEntityInterface $category)
    {
    ...
    protected function _fetchData(KViewContext $context)
    {
        ...
        {
            $menu->getParams()->set('combined_layout', true);
            $this->_setHybridData($context);
        }
        else
        {
            $menu->getParams()->set('combined_layout', false);
            $this->_setListData($context);
        }

        $this->_setSearchFilterData($context);
    }

    protected function _setHybridData(KViewContext $context)
    {
        ...
        $category_state = $state->getValues();
        unset($category_state['slug']);

        $model = $this->getObject('com://admin/docman.model.categories')->setState($category_state);

        if ($parent) {
            $model->parent_id($parent->id);
        }

        $model->level(1)->sort($params->sort_categories)->direction($params->direction_categories ?: 'asc');

        $categories = [];

        foreach ($model->fetch() as $category) {
        ...
        if (count($categories))
        {
            ...
            $model = $this->getObject('com://site/docman.controller.document')
                          ...
                          ->limit($state->limit)
                          ->offset($state->offset)
                          ->csort($params->sort_categories)
                          ->cdirection($params->direction_categories ?: 'asc')
                          ->sort($state->sort)
        ...
        // Call the generic _fetchData method
        ComDocmanViewHtml::_fetchData($context);

        $context->parameters->total = $total;
    }

    protected function _setListData(KViewContext $context)
    {
        $state  = $this->getModel()->getState();
        $params = $this->getParameters();
        $user   = $this->getObject('user');
        ...
        if (!$parent)
        {
            ...
            else $category = $this->getModel()->fetch();
        }
        ...
        if ($category->id || $has_filter)
        {
            ...
            $states = [
               ...
               'search_by'         => $params->search_by,
               'search_contents'   => $search_contents,
               'search_fields'     => $filter->search_fields,
               'tag'               => $filter->tag,
               'category'          => $document_category,
               'category_children' => $document_category_children
            ];

            $model     = $this->_buildModel($states);
            $total     = $model->count();
            $documents = $model->fetch();
        ...
        if (isset($states))
        {
            $context->data->states = $states;
            $this->_setAllDocumentsData($context);
        }
    }
    ...
    protected function _setSearchFilterData(KViewContext $context)
    {
        ...
        if (!empty($menu->query['own']) || empty($owner) || count($owner) <= 1) {
            $menu->getParams()->set('show_owner_filter', false);
        }
        ...
        if ($menu->getParams()->get('show_owner_filter')) {
    ...
    protected function _generatePathway($category = null, $document = null)
    {
        ...
        if ($model->getState()->isUnique() && !$this->getParentCategory()) {
            $category = $this->getModel()->fetch();
        } else {
            $category = null;
        }
        
        parent::_generatePathway($category);
    }
    ...
    protected function _buildModel(array $states =[])
    {
        $controller = $this->getObject('com://site/docman.controller.document');
