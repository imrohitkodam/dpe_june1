...
<div class="docman_document" itemscope itemtype="http://schema.org/CreativeWork">
            ...
            <span class="koowa_wrapped_content">
                <span class="whitespace_preserver">
                    ...
                    <? if ($document->canPerform('edit') && $document->isLockable() && $document->isLocked()): ?>
                        <span class="label label-warning badge bg-dark"><?= helper('grid.lock_message', array('entity' => $document)); ?></span>
                    <? endif; ?>
    ...
    <p class="docman_document_details">
        ...
        <? if ($params->show_document_category):
            ...
            <span class="category-label">
                <? // @TODO Use the active menu instead of accessing the request object directly in the view ?>
    ...
    <? if(!$params->force_download && $params->embed_document && $document->isViewable()): ?>
    <p>
        <?= helper('viewer.render', array('document' => $document)) ?>
    </p>
    <? endif; ?>

    <?
    // Show download button business rules
    $show_download = false;

    if ($params->embed_document && ($document->isPlayable() || $document->isViewable())) {
        $show_download = false;
    } else {
        $show_download = true;
    }

    $show_download = $show_download && (!object('user')->isAuthentic() || $document->canPerform('download'));
    ?>
    ...
    <div class="docman_description">
        ...
        ?>
            ...
            <?= prepareText($document->$field, $document); ?>
            </div>
        <? endif; ?>
    </div>
    <? endif ?>
