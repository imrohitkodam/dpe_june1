...
class ComDocmanRouter
{
    ...
    public function build(&$query)
    {
        ...
        if (empty($query['Itemid']) || !empty($query['itemless'])) {
            return $segments;
        }

        $menu = JApplicationCms::getInstance('site')->getMenu()->getItem($query['Itemid']);
        ...
        $menu_option = isset($menu_query['option']) ? $menu_query['option'] : null;
        ...
        elseif ($query_view == 'search' && !($menu_option == 'com_docman' && $menu_view == 'search')) unset($query['Itemid']); // Remove default Itemid being appended by Joomla on search module context
        ...
        if ($query_view !== 'search' && ($menu_view === $query_view || !isset($query['layout']) || $query['layout'] !== 'form')) {
            unset($query['view']);
        }

        // Do not remove layout from query for standalone search requests
        if (!($query_view == 'search' && !($menu_option == 'com_docman' && $menu_view == 'search')) && ($query_layout === 'default' || $query_layout === $menu_layout)) {
            unset($query['layout']);
        }
        ...
        if ($menu_view == 'list')
        {
            $removables = ['combined', 'own_categories'];

            foreach ($removables as $removable) {
                if (isset($query[$removable])) unset($query[$removable]);
            }
        }

        // Top level mod_menu link has own=X in the link
        if (version_compare(JVERSION, '4', '>=')) {
            if (isset($query['own']) && isset($menu_query['own']) && $query['own'] === $menu_query['own']) {
                unset($query['own']);
            }
        }
    ...
    public function parse(&$segments)
    {
        ...
        $view      = \Joomla\CMS\Factory::getApplication()->input->getCmd('view', null);
        ...
        {
            $vars['view'] = isset($menu->query['view']) ? $menu->query['view'] : null;
            $vars['slug'] = array_pop($segments);

            // Ensure valid slug
            $vars['slug'] = $vars['slug'] == 'index.php' ? '' : $vars['slug'];
        ...
        // Joomla complains if we leave any segments
        $segments = [];
