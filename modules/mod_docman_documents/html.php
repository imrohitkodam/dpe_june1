...
class ModDocman_documentsHtml extends ModKoowaHtml
{
    ...
    protected function _actionRender(KViewContext $context)
    {
        ...
        // Gallery document layout requires that document_title_link prop is set, hardcoding value if module settings are set to download

        if ($this->getLayout() == 'gallery' && $params->link_to_download) {
    ...
    protected function _beforeRender(KViewContext $context)
    {
        ...
        foreach($params->toArray() as $param => $value)
        {
            if (substr($param, 0, 7) == 'slider_')
            {
                ...
                $option = str_replace('slider_', '', $param);
                $options[$option] = $value;
                
                // Build video sub settings for the slider helper
                if (substr($option, 0, 6) == 'video_')
                {
                    $options['video'][str_replace('video_', '', $option)] = $value;
    ...
    protected function _prepareDocuments($documents, $params)
    {
        ...
        foreach ($documents as $document)
        {
            ...
            if (!empty($current))
            {
                if (!$params->link_to_download) {
                    $document->document_title_link = $current->params->get('document_title_link'); // Set title link behavior on document based on menu item settings
                }
                ...
                if ($params->show_category)
                {
                    ...
                    if (isset($current->query['view']) && in_array($current->query['view'], array('tree', 'list')))
                    {
