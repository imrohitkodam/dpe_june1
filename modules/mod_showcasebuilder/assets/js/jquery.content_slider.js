...
function getBaseURL() {
    return document.URL.substring(0, document.URL.nthIndex(5,"/") + 1);
}



var modulespath = getBaseURL()+'modules/mod_showcasebuilder/assets/';
//FINE DEFINIZIONE PATH SITO
...
// ----------------- Content Slider main class ---------------------------
	function content_slider(element, options) {
		...
		//alert (this.options.description);
	...
	content_slider.prototype = {
		...
		create_html: function () {
			...
			var i, img, real_i;
			var html_code = '<div class="'+this.options.container_class+'"><div class="image_more_info"><a href="#"><img alt="" src="'+this.options.plugin_url+modulespath+'images/more.png"></a></div>';
			this._start = -1;
		...
		_create_arrows : function () {
			...
			if (this.options.hv_switch) {
				if (this.options.border_on_off==0 || this.options.use_thin_arrows==1) {
				html = '<div class="circle_slider_nav_left"><img src="'+this.options.plugin_url+modulespath+'images/left_vertical2.png" alt="left" /></div>';
					html += '<div class="circle_slider_nav_right"><img src="'+this.options.plugin_url+modulespath+'images/right_vertical2.png" alt="right" /></div>';
				} else {
					html = '<div class="circle_slider_nav_left"><img src="'+this.options.plugin_url+modulespath+'images/left_vertical.png" alt="left" /></div>';
					html += '<div class="circle_slider_nav_right"><img src="'+this.options.plugin_url+modulespath+'images/right_vertical.png" alt="right" /></div>';
					}
			} else {
				if (this.options.border_on_off==0 || this.options.use_thin_arrows==1) {
					html = '<div class="circle_slider_nav_left"><img src="'+this.options.plugin_url+modulespath+'images/left2.png" alt="left" /></div>';
					html += '<div class="circle_slider_nav_right"><img src="'+this.options.plugin_url+modulespath+'images/right2.png" alt="right" /></div>';
				} else {
					html = '<div class="circle_slider_nav_left"><img src="'+this.options.plugin_url+modulespath+'images/left.png" alt="left" /></div>';
					html += '<div class="circle_slider_nav_right"><img src="'+this.options.plugin_url+modulespath+'images/right.png" alt="right" /></div>';
				}
			}
			return html;
		},
		...
		_set_arrows_events : function () {
			...
			this.$prettyPhoto_img.click (function (e){
				...
				if (rel=='prettyPhoto') {
					...
					//GIANLUCA PASSAGGIO DI VARIABILI
					var titleitem = self.title;
					var contentHtml = self.content;
					//GIANLUCA PASSAGGIO DI VARIABILI
				
					$.prettyPhoto.open(href,titleitem,contentHtml);
		...
		show_text: function (middle, dont_fadein, set_width) {
			...
			//GIANLUCA - PASSAGGIO VARIABILI
			this.title=this.options.title[middle].title;
			this.content=this.options.content[middle].content;
			//GIANLUCA - PASSAGGIO VARIABILI
