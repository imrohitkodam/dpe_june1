...
const nodes=[],links=[];jmapKnowledgeGraphData.forEach(item=>{let q=item.question?.trim(),a=item.answer?.trim(),img=item.image?.trim(),questionNode=q?{id:q,type:"question",url:item.url}:null,answerNode=a?{id:a,type:"answer"}:null,imageNode=img?{id:img,type:"image",imgUrl:img}:null; if(questionNode) nodes.push(questionNode); if(answerNode) nodes.push(answerNode); if(imageNode) nodes.push(imageNode); if(questionNode && answerNode) links.push({source:q,target:a}); if(questionNode && imageNode) links.push({source:q,target:img});}); const width=800,height=600,svg=d3.select("div.jmap-module-aiknowledgegraph svg").attr("viewBox",`0 0 ${width} ${height}`),zoomable=svg.append("g"),container=document.querySelector(".jmap-module-aiknowledgegraph"),containerRect=container.getBoundingClientRect(),tooltip=d3.select("div.jmap-module-aiknowledgegraph .jmap-module-aiknowledgegraph-tooltip"),simulation=d3.forceSimulation(nodes).force("link",d3.forceLink(links).id(d=>d.id).distance(180)).force("charge",d3.forceManyBody().strength(-300)).force("center",d3.forceCenter(width/2,height/2)),link=zoomable.append("g").selectAll("line").data(links).enter().append("line").attr("class","link"),node=zoomable.append("g").selectAll("g").data(nodes).enter().append("g"); node.append("a").attr("xlink:href",d=>d.type==="question"?d.url:null).attr("target","_blank").each(function(d){const group=d3.select(this); if(d.type==="image"){group.append("image").attr("xlink:href",d.imgUrl).attr("width",30).attr("height",30).attr("x",-15).attr("y",-15)} else {group.append("circle").attr("r",d.type==="question"?12:8).attr("fill",d.type==="question"?"orange":"steelblue")}}).on("mouseover",(event,d)=>{tooltip.style("visibility","visible").text(d.id)}).on("mousemove",(event)=>{tooltip.style("top",(event.clientY-containerRect.top+10)+"px").style("left",(event.clientX-containerRect.left+10)+"px")}).on("mouseout",()=>{tooltip.style("visibility","hidden")}); node.append("text").attr("dy",-15).text(d=>d.id.length>jmapNodesLength?d.id.substring(0,jmapNodesLength)+"...":d.id); node.call(d3.drag().on("start",(event,d)=>{if(!event.active){simulation.alphaTarget(0.3).restart(); d.fx=d.x; d.fy=d.y}}).on("drag",(event,d)=>{d.fx=event.x; d.fy=event.y}).on("end",(event,d)=>{if(!event.active)simulation.alphaTarget(0); d.fx=null; d.fy=null})); const zoom=d3.zoom().scaleExtent([0.5,5]).on("zoom",event=>{zoomable.attr("transform",event.transform)}); svg.call(zoom); simulation.on("tick",()=>{link.attr("x1",d=>d.source.x).attr("y1",d=>d.source.y).attr("x2",d=>d.target.x).attr("y2",d=>d.target.y); node.attr("transform",d=>`translate(${d.x}, ${d.y})`)});
