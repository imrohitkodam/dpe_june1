...
if(!defined('OfflajnSearchThemeCache')) {
  ...
  class OfflajnSearchThemeCache extends Registry{
    ...
    function __construct($namespace, &$_module, &$_params){
      ...
      $this->cachePath = Path::clean(JPATH_SITE.'/modules/'.$this->module->module.'/cache/'.$folder.'/');
      if(!Folder::exists($this->cachePath)) Folder::create($this->cachePath);
      if(!Folder::exists($this->cachePath)){
        $writeError = true;
      }
      if($writeError){
        Text::printf("%s is unwriteable or non-existent, because the system does not allow the operation from PHP. Please create the directory and set the writing access!", $this->cachePath);
        exit;
      }
    ...
    */
    ...
    function generateCSSCache(){
      ...
      if(!is_file($file)){
        $needToDelete = Folder::files($this->cachePath, '(css)|(png)|(jpg)|(svg)$', false, true);
    ...
    function generateJSCache(){
      ...
      if(!is_file($file)){
        $needToDelete = Folder::files($this->cachePath, 'js$', false, true);
        if(is_array($needToDelete) && count($needToDelete) > 0){
