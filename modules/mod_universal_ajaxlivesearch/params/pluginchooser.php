...
class JElementPluginChooser extends JOfflajnFakeElementBase
{
    ...
    foreach ($plgs as $plg){
      ...
      $html_value = isset($pluginNames[$plg[0]]) ? $pluginNames[$plg[0]] : Text::_($plg[1]);

		  $html[] = '<input type="checkbox" name="'.$name.'[]" value="'.$plg[0].'" id="'.$name.'_'.$plg[0].'" '.$checked.' />';
			$html[] = ' <label for="'.$id.'_'.$plg[0].'"  style="clear:none; display:inline-block;min-width: 120px;" >'.Text::_($plg[1]).'</label>';
      ...
      $html[] = ' <a href="'.Route::_('index.php?option=com_plugins&task=plugin.edit&extension_id='.$plg[0]).'" target="_blank">Open plugin settings</a> ';
...
if(version_compare(JVERSION,'1.6.0','ge')) {
  class FormFieldPluginChooser extends JElementPluginChooser {}
}
