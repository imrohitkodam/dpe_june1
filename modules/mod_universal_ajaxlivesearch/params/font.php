...
class JElementFont extends JOfflajnFakeElementBase
{
  var $_moduleName = '';
    ...
    $h = '
<fieldset style="position:relative; top: 20%; margin: 0 auto; width: 740px; background-color: #fff;border-radius: 8px; box-shadow: 0 0 5px rgba(0,0,0,0.4);">
  ...
  <table class="fontchooser" style="width: 100%;font-weight: bold;">
    ...
    ';
		$html.="<a style='float: left;' id='".$id."change' href='#'>[".Text::_('Change font settings')."]</a>&nbsp;&nbsp;";
    ...
    $google = Folder::files($p, '.txt');
    foreach($google as $g){
      $this->_google[] = File::stripExt($g);
      preg_match_all('/((?:^|[A-Z])[a-z]+)/',File::stripExt($g),$matches);
      $this->_googleName[] = implode(' ', $matches[1]);
      $fp = @fopen($p.$g, 'r');
      if ($fp) {
        $this->_googlefonts[File::stripExt($g)] = explode("\r\n", fread($fp, filesize($p.$g)));
      }
      fclose($fp);
    }
  }
    ...
    $chooser = "
    <select id='fonttype'>
      <option value='0'>". Text::_('Use only the alternative fonts') ."</value>
      <optgroup label='Google fonts'>
    ";
    foreach($this->_google AS $k => $g){
      $chooser.= "<option value='".$g."'>". Text::_($this->_googleName[$k]) ."</option>";
    }
    $chooser.= "
      </optgroup>
    </select>
...
if(version_compare(JVERSION,'1.6.0','ge')) {
  class FormFont extends JElementFont {}
}