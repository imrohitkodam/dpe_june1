...
class ModScloginHelper
{
    ...
    function __construct($params)
    {
        ...
        if($this->isJFBConnectInstalled && JFBCFactory::config()->get('jquery_load'))
            $this->jfbcBsVersion = JFBCFactory::config()->get('bootstrap_css');
        else
            $this->jfbcBsVersion = "0";

        if(JVERSION >= 4.0 || $this->jfbcBsVersion == '2')
        {
            $this->bsVersion = "";
            $this->bsClass = "ns-bs5";
    ...
    public function getButtonClass($btnParameter)
    {
        $val = $this->params->get($btnParameter);
        if(JVERSION < 4.0 && $this->jfbcBsVersion != '2')
        {
            if(empty($val))
                return 'btn';
    ...
    public function getForgotStyle($displaySetting, $styleSetting)
    {
        $style = $this->params->get('forgotusername_button_class');

        $display = $this->params->get($displaySetting);
...
// common for J!, JomSocial, and Virtuemart
    ...
    {
        return $this->getForgotButton($this->params->get('showForgotUsername'),
            $this->params->get('forgotusername_button_class'),
            $this->forgotLink, $this->forgotUsernameLink,
            Text::_('MOD_SCLOGIN_FORGOT_LOGIN'), Text::_('MOD_SCLOGIN_FORGOT_USERNAME'), $this->params->get('register_type'));
    }

    function getForgotPasswordButton()
    {
        return $this->getForgotButton($this->params->get('showForgotPassword'),
            $this->params->get('forgotpassword_button_class'),
            $this->forgotLink, $this->forgotPasswordLink,
            Text::_('MOD_SCLOGIN_FORGOT_LOGIN'), Text::_('MOD_SCLOGIN_FORGOT_PASSWORD'), $this->params->get('register_type'));
    }

    private function getForgotButton($showForgotButtonType, $forgotButtonStyle,
                                     $forgotSharedLink, $forgotButtonLink,
                                     $forgotSharedText, $forgotButtonText, $registerType)
    {
        $forgotButton = '';
        if($showForgotButtonType == 'button')
        {
            ...
            {
                $text = $forgotSharedText;
                $link = $forgotSharedLink;
            }
            else
            {
                $text = $forgotButtonText;
                $link = $forgotButtonLink;
            }

            $forgotButton = '<span class="input-group-text btn '.$forgotButtonStyle.'">'
                . '<a href="' . $link . '" class="forgot hasTooltip" tabindex="-1" data-placement="right" title="' . $text . '" aria-label="'.$text.'">
                                <i class="'. $question . '"></i>'
                .'</a>'
                . '</span>';
...
EOT;
                $showButton .=
                    '<a class="showpasswd" onclick="' . $fnName . '()">'
                    .'<span class="' . $eye . '" id="sc-eye-' . $passwordId . '"></span>'
                    .'</a>';
            }
            else
            {
                $showButton = '<label for="' . $passwordId . '" class="visually-hidden">' . Text::_('MOD_SCLOGIN_PASSWORD') . '</label>'
                    . '<a type="button" class="showpasswd input-password-toggle">'
    ...
    function getForgotLinks()
    {
        ...
        if($this->params->get('showForgotUsername') == 'link')
            $linksToShow[] = '<li><span class="btn-link"><a href="' . $this->forgotUsernameLink . '">' . Text::_('MOD_SCLOGIN_FORGOT_USERNAME') . '</a></span></li>';
        if($this->params->get('showForgotPassword') == 'link')
            $linksToShow[] = '<li><span class="btn-link"><a href="' . $this->forgotPasswordLink . '">' . Text::_('MOD_SCLOGIN_FORGOT_PASSWORD') . '</a></span></li>';
    ...
    function getUserMenu($userMenu, $menuStyle, $menuTitle='1')
    {
        ...
        if (!empty($menu_items))
        {
            ...
            else //Show in Bootstrap dropdown list
            {
                ...
                $menuNav .= '<a class="btn dropdown-toggle" data-bs-toggle="dropdown" data-toggle="dropdown" href="#" role="button">' . $parentTitle . '<span class="caret"></span></a>';
                $menuNav .= '<ul class="dropdown-menu">';
                foreach ($menu_items as $menuItem)
                {
                    if($menuItem->getParams()->get('menu_show', '1'))
