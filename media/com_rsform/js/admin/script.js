...
function getValidationExtraText(value)
{
	...
	switch (value)
	{
		case 'restrictedwords':
			...
			break;
		case 'restrictedtext':
			theText = Joomla.JText._('RSFP_COMP_FIELD_VALIDATIONEXTRARESTRICTEDTEXT');
			break;
		case 'regex':
...
function changeValidation(elem) {
	...
	if (elem.id === 'VALIDATIONRULE') {
		...
		if (extraId) {
			...
			if (['custom', 'numeric', 'alphanumeric', 'alpha', 'alphaaccented', 'alphanumericaccented', 'regex', 'sameas', 'minlength', 'maxlength', 'restrictedwords', 'restrictedtext'].indexOf(elem.value) > -1) {
		...
		for (i = 0; i < elem.length; i++) {
			if (elem[i].selected && ['custom', 'numeric', 'alphanumeric', 'alpha', 'alphaaccented', 'alphanumericaccented', 'regex', 'sameas', 'minlength', 'maxlength', 'restrictedwords', 'restrictedtext'].indexOf(elem[i].value) > -1) {
