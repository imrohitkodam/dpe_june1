        ...
        onChangefun: function() {
            jQuery("#venue_gallary_filter").change(function() {
                ...
                if (filterVal == "1") {
                    ...
                    jQuery("#venue_videos").show();
                    jQuery("#venue_images").show();
                    jQuery(".videosText").text(Joomla.JText._('COM_JTICKETING_VENUE_VIDEOS'));
        ...
        initEventJs: function() {
            ...
            Joomla.submitbutton = function(task) {
                ...
                if(task == "event.save" || task == "event.save2new" || task == "event.apply")
                {
                   ...
                   if(id != '0')
                   {
                        if(oldEventStartDate != eventStartDate || oldEventEndDate != eventEndDate)
                        {
                        ...
                        while(1)
                        {
                            ...
                            if (jQuery(unlimited).val() == 0) {
                                if(jQuery(available).val() <= 0 || !/^\d+$/.test(jQuery(available).val())) {
                                    ...
                                    jQuery("#system-message-container").html("<button type='button' class='close' data-dismiss='alert'>×</button><div class='alert alert-error'>" + error_html + "</div>");
    ...
    couponform: {
        ...
        couponSubmitButton: function(task) {
            if (task == 'couponform.cancel') {
                ...
                if (task != 'couponform.cancel' && document.formvalidator.isValid(document.id('coupon-form'))) {
                    Joomla.submitform(task, document.getElementById('coupon-form'));
                }
            }
        },
    ...
    venue: {
        ...
        venueSubmitButton: function(task) {
            if (task == 'venue.apply' || task == 'venue.save' || task == 'venue.save2new') {
                ...
                if (!document.formvalidator.isValid('#venue-form')) {
                        return false;
                }
                if (jQuery('input[name="jform[online]"]:checked').val() == 1) {
                    if (!onlineProvider || onlineProvider == '0') {
...
var tjMediaFile = {
    ...
    tjMediaGallery: {
        appendMediaToGallary: function(mediaData) {
            ...
            if (type[0] === 'video') {
                if (type[1] === 'youtube') {
                    mediaTag = "<iframe width='160' height='113' src=" + mediaData.path + "> </iframe>";
                } else {
                    mediaTag = "<video class='media_video_width' preload='metadata' controls ><source src=" + mediaData.path + "></video>";
                }
