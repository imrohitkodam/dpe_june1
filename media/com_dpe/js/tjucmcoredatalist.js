	...
	itmes: {
		...
		init: function () {
			jQuery(document).ready(function () {
				jQuery("#ropProcessCheck1").click(function () {
		...
		openVendorPopups: function (url, element = '') {
			ParentIframeElement = window.parent.document.getElementsByClassName('shadow tjucm-addprocess-doc').item(0);
			ParentIframeElement.classList.add('create-new-parent');
			...
			SqueezeBox.open(url, {
				...
				classWindow: 'tjucm-addvendor-pop',
			});
		},
		openNonVendorPopups: function (url, element = '') {
			SqueezeBox.open(url, {
				handler: 'iframe',
				size: {
					x: window.innerWidth - 200,
