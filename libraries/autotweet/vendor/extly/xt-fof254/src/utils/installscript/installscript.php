...
abstract class XTF0FUtilsInstallscript
{
	...
	public function postflight($type, $parent)
	{
		...
		$dbInstaller = new XTF0FDatabaseInstaller(array(
	...
	protected function copyCliFiles($parent)
	{
		...
		{
			if (file_exists($cliPath . '/' . $script))
			{
				unlink($cliPath . '/' . $script);
			}

			if (file_exists($src . '/' . $this->cliSourcePath . '/' . $script))
			{
				copy($src . '/' . $this->cliSourcePath . '/' . $script, $cliPath . '/' . $script);
			}
		}
	}
	...
	{
		$db = $db = XTF0FPlatform::getInstance()->getDbo();
