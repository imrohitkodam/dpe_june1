...
class XTF0FModel extends XTF0FUtilsObject
{
	...
	public static function &getAnInstance($type, $prefix = '', $config = array())
	{
		...
		if (array_key_exists('input', $config))
		{
			if (!($config['input'] instanceof \Joomla\Input\Input))
			{
				...
				$config['input'] = new \Joomla\Input\Input($config['input']);
			}
		}
		else
		{
	...
	{
		$defSource = array(['cleared-input' => true]);
		$this->input = new \Joomla\Input\Input($defSource);
	...
	public function setInput($input)
	{
		if (!($input instanceof \Joomla\Input\Input))
		{
			...
			$input = new \Joomla\Input\Input($input);
		}
	...
	public function publish($publish = 1, $user = null)
	{
		if (is_array($this->id_list) && !empty($this->id_list))
		{
			...
			else
			{
				...
				$result  = XTF0FPlatform::getInstance()->runPlugins($this->event_change_state, array($context, $this->id_list, $publish));
			}
		}
	...
	protected function preprocessForm(XTF0FForm &$form, &$data, $group = 'content')
	{
		...
		$results = XTF0FPlatform::getInstance()->runPlugins('onContentPrepareForm', array($form, $data));
	...
	protected function onBeforeSave(&$data, &$table)
	{
		...
		try
		{
			...
			// Call the plugin
			$name = $this->name;
			$result = XTF0FPlatform::getInstance()->runPlugins($this->event_before_save, array($this->option . '.' . $name, &$table, $this->_isNewRecord, $data));
	...
	protected function onAfterSave(&$table)
	{
		...
		try
		{
			...
			$name = $this->name;
			XTF0FPlatform::getInstance()->runPlugins($this->event_after_save, array($this->option . '.' . $name, &$table, $this->_isNewRecord));
	...
	protected function onBeforeDelete(&$id, &$table)
	{
		...
		try
		{
			...
			$name = $this->name;
			$context = $this->option . '.' . $name;
			$result = XTF0FPlatform::getInstance()->runPlugins($this->event_before_delete, array($context, $table));
	...
	protected function onAfterDelete($id)
	{
		...
		try
		{
			$name = $this->name;
			$context = $this->option . '.' . $name;
			$result = XTF0FPlatform::getInstance()->runPlugins($this->event_after_delete, array($context, $this->_recordForDeletion));
