...
class XTF0FDispatcher extends XTF0FUtilsObject
{
	...
	/** @var \Joomla\Input\Input Input variables */
	...
	public static function &getTmpInstance($option = null, $view = null, $config = array())
	{
		if (array_key_exists('input', $config))
		{
			...
			else
			{
				...
				$input = new \Joomla\Input\Input($config['input']);
			}
		}
		else
		{
	...
	public function dispatch()
	{
		...
		$controller = XTF0FController::getTmpInstance($option, $view, $config);
	...
	protected function getTask($view)
	{
		...
		switch ($requestMethod)
		{
			...
			default:
				...
				elseif (($task == 'edit') && XTF0FPlatform::getInstance()->isFrontend())
				{
					$task = 'read';
				}
				break;
	...
	public function transparentAuthentication()
	{
		...
		foreach ($this->fofAuth_AuthMethods as $method)
		{
			...
			$authInfo = $this->processMethod($method);
	...
	private function _decryptWithTOTP($encryptedData)
	{
		...
		$totp = new XTF0FEncryptTotp($this->fofAuth_timeStep);
