...
class XTF0FController extends XTF0FUtilsObject
{
	...
	public static function &getTmpInstance($option = null, $view = null, $config = array())
	{
		...
		{
			$input = new \Joomla\Input\Input($input, $input_options);
		}
	...
	public function __construct($config = array())
	{
		...
		{
			$this->input = new \Joomla\Input\Input($input, $input_options);
		}
		...
		$classNameParts = XTF0FInflector::explode(get_class($this));
	...
	{
		XTF0FPlatform::getInstance()->logDeprecated(__CLASS__ . '::' .__METHOD__ . ' is deprecated. Use checkACL() instead.');
	...
	public function execute($task)
	{
		...
		if ($result)
		{
			...
			$plugin_result = XTF0FPlatform::getInstance()->runPlugins($plugin_event, array(&$this, &$this->input));
		...
		if ($result)
		{
			...
			$plugin_result = XTF0FPlatform::getInstance()->runPlugins($plugin_event, array(&$this, &$this->input, &$ret));
	...
	public function display($cachable = false, $urlparams = false, $tpl = null)
	{
		...
		if (XTF0FPlatform::getInstance()->isFrontend() && $cachable && ($viewType != 'feed') && $conf->get('caching') >= 1)
		{
			// Get a JCache object
	...
	public function edit()
	{
		...
		if (!$status)
		{
			...
			$url = !empty($customURL) ? $customURL : 'index.php?option=' . $this->component . '&view=' . XTF0FInflector::pluralize($this->view) . $this->getItemidURLSuffix();
	...
	final public function getThisModel($config = array())
	{
		if (!is_object($this->_modelObject))
		{
			...
			if (!array_key_exists('input', $config) || !($config['input'] instanceof \Joomla\Input\Input))
			{
	...
	{
		XTF0FPlatform::getInstance()->logDeprecated(__CLASS__ . '::' .__METHOD__ . ' is deprecated. Use createModel() instead.');
	...
	{
		XTF0FPlatform::getInstance()->logDeprecated(__CLASS__ . '::' . __METHOD__ . ' is deprecated. Use createView() instead.');
	...
	{
		if (XTF0FPlatform::getInstance()->isFrontend() && ($this->input->getCmd('Itemid', 0) != 0))
		{
