...
class XTF0FTable extends XTF0FUtilsObject implements JTableInterface
{
	...
	public static function &getAnInstance($type = null, $prefix = 'JTable', $config = array())
	{
		...
		{
			$config['input'] = new \Joomla\Input\Input($_REQUEST);
		}

		if ($config['input'] instanceof \Joomla\Input\Input)
		{
			$tmpInput = $config['input'];
		}
		else
		{
			$tmpInput = new \Joomla\Input\Input($config['input']);
		}
	...
	public static function forceInstance($key = null, $instance = null)
	{
		...
		elseif($key && isset(self::$instances[$key]))
		{
			...
			if(!$instance || ($instance && $instance instanceof XTF0FTable))
			{
	...
	public function __construct($table, $key, &$db, $config = array())
	{
		...
		if (array_key_exists('input', $config))
		{
			...
			{
				$this->input = new \Joomla\Input\Input($config['input']);
			}
		}
		else
		{
			$this->input = new \Joomla\Input\Input($_REQUEST);
		}
	...
	public function checkout($userId, $oid = null)
	{
		...
		$date = XTF0FPlatform::getInstance()->getDate();
	...
	protected function onBeforeBind(&$from)
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onBeforeBind' . ucfirst($name), array(&$this, &$from));
	...
	protected function onAfterLoad(&$result)
	{
		...
		if ($this->_trigger_events)
		{
			...
			XTF0FPlatform::getInstance()->runPlugins('onAfterLoad' . ucfirst($name), array(&$this, &$result));
		}
	}
	...
	protected function onBeforeStore($updateNulls)
	{
		...
		{
			$name       = XTF0FInflector::pluralize($this->getKeyName());
			$result     = XTF0FPlatform::getInstance()->runPlugins('onBeforeStore' . ucfirst($name), array(&$this, $updateNulls));
	...
	protected function onAfterBind(&$src)
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onAfterBind' . ucfirst($name), array(&$this, &$src));
	...
	protected function onAfterStore()
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onAfterStore' . ucfirst($name), array(&$this));
	...
	protected function onBeforeMove($updateNulls)
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onBeforeMove' . ucfirst($name), array(&$this, $updateNulls));
	...
	protected function onAfterMove()
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onAfterMove' . ucfirst($name), array(&$this));
	...
	protected function onBeforeReorder($where = '')
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onBeforeReorder' . ucfirst($name), array(&$this, $where));
	...
	protected function onAfterReorder()
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onAfterReorder' . ucfirst($name), array(&$this));
	...
	protected function onBeforeDelete($oid)
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onBeforeDelete' . ucfirst($name), array(&$this, $oid));
	...
	protected function onAfterDelete($oid)
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onAfterDelete' . ucfirst($name), array(&$this, $oid));
	...
	protected function onBeforeHit($oid, $log)
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onBeforeHit' . ucfirst($name), array(&$this, $oid, $log));
	...
	protected function onAfterHit($oid)
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onAfterHit' . ucfirst($name), array(&$this, $oid));
	...
	protected function onBeforeCopy($oid)
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onBeforeCopy' . ucfirst($name), array(&$this, $oid));
	...
	protected function onAfterCopy($oid)
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onAfterCopy' . ucfirst($name), array(&$this, $oid));
	...
	protected function onBeforePublish(&$cid, $publish)
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onBeforePublish' . ucfirst($name), array(&$this, &$cid, $publish));
	...
	protected function onAfterReset()
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onAfterReset' . ucfirst($name), array(&$this));
	...
	protected function onBeforeReset()
	{
		...
		if ($this->_trigger_events)
		{
			...
			$result     = XTF0FPlatform::getInstance()->runPlugins('onBeforeReset' . ucfirst($name), array(&$this));
	...
	/**
	 * Replace the input object of this table with the provided \Joomla\Input\Input object
	 *
	 * @param   \Joomla\Input\Input  $input  The new input object
	 *
	 * @return  void
	 */
