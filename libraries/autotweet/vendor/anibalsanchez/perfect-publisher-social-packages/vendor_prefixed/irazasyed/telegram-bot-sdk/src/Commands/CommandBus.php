...
use XTS_BUILD\Telegram\Bot\Traits\Singleton;
use XTS_BUILD\Telegram\Bot\Answers\AnswerBus;
...
class CommandBus extends AnswerBus
{
    ...
    public function parseCommand($text, $offset, $length): string
    {
        ...
        if (XTS_str_contains($command, '@') && XTS_ends_with($command, ['bot', 'Bot'])) {
    ...
    {
        return XTS_collect($message->get('entities'))
    ...
    private function resolveCommand($command)
    {
        ...
        if (! ($command instanceof CommandInterface)) {
            throw new TelegramSDKException(
                sprintf(
                    'Command class "%s" should be an instance of "XTS_BUILD\Telegram\Bot\Commands\CommandInterface"',
                    get_class($command)
                )
            );
        }
