...
use XTS_BUILD\DirkGroenen\Pinterest\Exceptions\PinterestException;

use \XTS_BUILD\DirkGroenen\Pinterest\Pinterest;
use \XTS_BUILD\DirkGroenen\Pinterest\Transport\Response;
...
class Collection implements \JsonSerializable, \ArrayAccess, \IteratorAggregate{
    ...
    public function __construct(Pinterest $master, $items, $model) {
        ...
        if (!class_exists("\\XTS_BUILD\\DirkGroenen\\Pinterest\\Models\\" . $this->model)) {
        ...
        if (is_array($items)) {
            $this->response = null;
            $this->items = $items;
        } else if ($items instanceof \XTS_BUILD\DirkGroenen\Pinterest\Transport\Response) {
            $this->response = $items;
    ...
    private function buildCollectionModels(array $items)
    {
        ...
        foreach ($items as $item) {
            $class = new \ReflectionClass("\\XTS_BUILD\\DirkGroenen\\Pinterest\\Models\\" . $this->model);
