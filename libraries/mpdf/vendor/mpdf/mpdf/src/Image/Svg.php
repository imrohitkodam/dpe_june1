...
//*/
	...
	function Arcto($x1, $y1, $x2, $y2, $rx, $ry, $angle, $largeArcFlag, $sweepFlag)
	{
		...
		if ($numerator < 0.0) {
			...
			//    ry'=s*ry becomes 0 :
			if ($rx == 0 || $ry == 0) {
			    $s = 1.0; // Or handle as a special case — maybe skip or log an error
			} else {
...
$denominator = ($rx * $rx * $y1dash * $y1dash + $ry * $ry * $x1dash * $x1dash);

if ($denominator == 0.0) {
    $root = 0.0; // or handle according to your logic
} else {
    $root = ($largeArcFlag == $sweepFlag ? -1.0 : 1.0) * sqrt($numerator / $denominator);
}
		}

if ($ry == 0.0) {
    $cxdash = 0.0; // or handle this case appropriately (e.g., log, skip drawing, etc.)
} else {
    $cxdash = $root * $rx * $y1dash / $ry;
}
		if ($rx == 0) {
		    // Handle this scenario gracefully, e.g., skip calculation or log warning
		    $cydash = 0; // Or another safe fallback
		} else {
		...
		if ($rx == 0 || $ry == 0) {
		    // Handle invalid arc — either log, set $theta1 to a safe fallback, or skip drawing
		    $theta1 = 0; // Or whatever default behavior makes sense
		} else {
		    $theta1 = $this->CalcVectorAngle(1.0, 0.0, ($x1dash - $cxdash) / $rx, ($y1dash - $cydash) / $ry);
		}
if ($rx == 0 || $ry == 0) {
    // Handle invalid arc input — skip, log, or use fallback value
    $dtheta = 0; // Or handle according to your logic
} else {
...
if ($segments == 0) {
    $delta = 0; // or handle according to your use case
} else {
    $delta = $dtheta / $segments;
}

$denominator = sin($delta / 2.0);

if ($denominator == 0.0) {
    $t = 0; // or handle gracefully based on your rendering needs
} else {
    $t = 8.0 / 3.0 * sin($delta / 4.0) * sin($delta / 4.0) / $denominator;
}
		$coords = [];
